...
export async function executeSearch(
	...
	try {
		...
		// Complete progress
		progress.succeed(
			`Found ${results.length} repositories matching "${query}"`,
		);
		...
		if (options.json) {
			...
			results.forEach((result: SearchResult, index: number) => {
				...
				console.log(
					chalk.gray(`   Last Analyzed: ${formatDate(repo.lastAnalyzed)}`),
				);
				...
				if (result.matches.length > 0) {
					...
					result.matches.slice(0, 3).forEach((match) => {
						console.log(
							chalk.gray(
								`   - ${match.field}: ${match.value} (Score: ${match.score.toFixed(2)})`,
							),
						);
					});
