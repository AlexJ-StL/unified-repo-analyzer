...
describe("ConfigurationManager", () => {
	...
	describe("Configuration Validation", () => {
		...
		it("should validate file size parsing", () => {
			...
			expect(configManager.parseFileSize("1.5GB")).toBe(
				1.5 * 1024 * 1024 * 1024,
			);
...
describe("ConfigValidator", () => {
	describe("Logger Configuration Validation", () => {
		...
		it("should detect invalid log levels", () => {
			...
			expect(result.isValid).toBe(false);
			expect(result.errors.some((e) => e.code === "INVALID_LOG_LEVEL")).toBe(
				true,
			);
		});
		...
		it("should validate file output configuration", () => {
			...
			const result = ConfigValidator.validateLoggerConfig(
				configWithInvalidFile,
			);
			expect(result.isValid).toBe(false);
			expect(result.errors.some((e) => e.code === "MISSING_FILE_PATH")).toBe(
				true,
			);
			expect(
				result.errors.some((e) => e.code === "INVALID_MAX_SIZE_FORMAT"),
			).toBe(true);
			expect(result.errors.some((e) => e.code === "INVALID_MAX_FILES")).toBe(
				true,
			);
		});
		...
		it("should validate external output configuration", () => {
			...
			const result = ConfigValidator.validateLoggerConfig(
				configWithInvalidExternal,
			);
			expect(result.isValid).toBe(false);
			expect(
				result.errors.some((e) => e.code === "MISSING_EXTERNAL_ENDPOINT"),
			).toBe(true);
			expect(result.errors.some((e) => e.code === "INVALID_BATCH_SIZE")).toBe(
				true,
			);
			expect(
				result.errors.some((e) => e.code === "INVALID_FLUSH_INTERVAL"),
			).toBe(true);
		});
