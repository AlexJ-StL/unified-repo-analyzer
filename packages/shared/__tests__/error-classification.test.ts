...
describe("Error Classification System", () => {
  ...
  describe("ErrorClassifier", () => {
    describe("classifyError", () => {
      ...
      it("should classify permission denied errors correctly", () => {
        ...
        expect(classified.suggestions.some((s) => s.platform === "linux")).toBe(
          true
        );
      });
      ...
      it("should link parent and child errors", () => {
        ...
        const childError = errorClassifier.classifyError(
          "Child error",
          {},
          parentError.id
        );
    ...
    describe("getCorrelatedErrors", () => {
      ...
      it("should return correlated errors", () => {
        ...
        expect((correlation?.relatedErrors ?? []).map((e) => e.id)).toContain(
          error1.id
        );
        expect((correlation?.relatedErrors ?? []).map((e) => e.id)).toContain(
          error2.id
      ...
      it("should determine root cause correctly", () => {
        ...
        const criticalError = errorClassifier.classifyError(
          "System resource exhausted",
          context
        );
        const _mediumError = errorClassifier.classifyError(
          "Path too long",
          context
        );
    ...
    describe("resolveError", () => {
      it("should mark error as resolved", () => {
        ...
        const resolved = errorClassifier.resolveError(
          classified.id,
          "Fixed by user"
        );
  ...
  describe("ErrorFormatter", () => {
    ...
    describe("formatForConsole", () => {
      ...
      it("should limit suggestions based on options", () => {
        ...
        if (suggestionsSection) {
          const suggestionMatches = suggestionsSection
            .split("Learn More:")[0]
            .match(/^\s*\d+\./gm);
    ...
    describe("formatErrorSummary", () => {
      it("should format multiple errors as summary", () => {
        const error1 = errorClassifier.classifyError(
          "Critical error",
          {},
          undefined
        );
    ...
    describe("createUserFriendlyMessage", () => {
      it("should create user-friendly messages for common errors", () => {
        ...
        const friendlyMessage =
          errorFormatter.createUserFriendlyMessage(pathError);

        expect(friendlyMessage).toContain("/missing/path");
        expect(friendlyMessage).toContain("couldn't find");
        expect(friendlyMessage.length).toBeGreaterThan(
          pathError.message.length
        );
      });

      it("should fall back to original message for unknown errors", () => {
        const unknownError =
          errorClassifier.classifyError("Unknown error type");
        const friendlyMessage =
          errorFormatter.createUserFriendlyMessage(unknownError);
  ...
  describe("Integration Tests", () => {
    ...
    it("should handle error correlation across multiple operations", () => {
      ...
      const _permissionError = errorClassifier.classifyError(
        "Permission denied",
        {
          ...baseContext,
