...
import { ErrorCategory, ErrorCode, ErrorSeverity } from '../types/error-classification.js';
/**
...
export class ErrorClassifier {
   ...
   */
  createErrorResponse(classifiedError, includeContext = false) {
    return {
      error: {
   ...
   */
  getCorrelatedErrors(correlationId) {
   ...
   */
  determineErrorCodeAndCategory(message, context) {
   ...
   */
  determineSeverity(code, _category, _context) {
    // Critical errors
    if (
      [
   ...
   */
  generateErrorTitle(code, _category) {
    const titleMap = {
   ...
   */
  enhanceErrorMessage(originalMessage, code, context) {
   ...
   */
  generateErrorDetails(code, context) {
   ...
   */
  generateSuggestions(code, category, context) {
    const suggestions = [];
   ...
   */
  getLearnMoreUrl(code, _category) {
    const urlMap = {
      [ErrorCode.PATH_INVALID_FORMAT]:
   ...
   * Update correlation mapping
   */
  updateCorrelationMapping(correlationId, errorId) {
   ...
   */
  linkToParentError(parentErrorId, childErrorId) {
