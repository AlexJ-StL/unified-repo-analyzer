...
export class ErrorFormatter {
	...
	public formatForConsole(
		...
		// Error details
		lines.push(
			this.colorize("Error ID:", this.colors.gray, opts.useColors) +
				` ${error.id}`,
		);
		lines.push(
			this.colorize("Code:", this.colors.gray, opts.useColors) +
				` ${error.code}`,
		);
		lines.push(
			this.colorize("Category:", this.colors.gray, opts.useColors) +
				` ${error.category}`,
		);
		lines.push(
			this.colorize("Severity:", this.colors.gray, opts.useColors) +
				` ${error.severity}`,
		);
		lines.push(
			this.colorize("Time:", this.colors.gray, opts.useColors) +
				` ${error.timestamp.toISOString()}`,
		);

		if (error.context.path) {
			lines.push(
				this.colorize("Path:", this.colors.gray, opts.useColors) +
					` ${error.context.path}`,
			);
		}
		...
		if (opts.includeLearnMore && error.learnMoreUrl) {
			lines.push("");
			lines.push(
				this.colorize("📖 Learn More:", this.colors.blue, opts.useColors) +
					` ${error.learnMoreUrl}`,
			);
		}
	...
	public createUserFriendlyMessage(error: ClassifiedError): string {
		const messageTemplates: Partial<Record<ErrorCode, string>> = {
			...
			[ErrorCode.NETWORK_TIMEOUT]: `The network request timed out. Please check your connection and try again.`,
