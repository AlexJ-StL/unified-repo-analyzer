...
export class BuildExecutor {
	...
	private initializeBuildContext(): void {
		...
		try {
			...
			this.log(
				"info",
				`Discovered ${this.buildContext.packages.length} packages`,
			);
		} catch (error) {
			this.log(
				"error",
	...
	private async handleBuildFailure(
		...
		const enhancedError = this.errorHandler.handleBuildError(
			buildError,
			context,
		);
		suggestions.push(...enhancedError.suggestions.map((s) => s.action));
	...
	private analyzeDependencyError(stderr: string): DependencyErrorContext {
		...
		const conflictMatch = stderr.match(
			/conflicting peer dependency:\s*([^,\n]+)/gi,
		);
		if (conflictMatch) {
		...
		for (const packageInfo of this.buildContext.packages) {
			...
			if (!packageInfo.hasBuildScript) {
				EnhancedLogger.logWarning(
					`Skipping ${packageInfo.name} - no build script`,
				);
				continue;
			}
	...
	private async generateBuildReport(
		...
		const report = {
			...
			success,
			duration: Object.values(results).reduce(
	...
	private generateRecoverySuggestions(
		...
		const suggestions: string[] = [];
		const failedPackages = Object.entries(results).filter(
		...
		const hasTypeScriptErrors = failedPackages.some(
			([, result]) =>
				result.stderr?.includes("TS") || result.error?.includes("TypeScript"),
		);
	...
	private printBuildSummary(
		...
		const successfulPackages = Object.values(results).filter(
			(r) => r.success,
		).length;
	...
	async validateBuildEnvironment(): Promise<{
		...
		if (majorVersion < 18) {
			issues.push(
				`Node.js version ${nodeVersion} is below minimum required (18.0.0)`,
			);
		}
		...
		const tsConfigExists = existsSync(
			join(this.buildContext.projectRoot, "tsconfig.json"),
		);
		if (!tsConfigExists) {
