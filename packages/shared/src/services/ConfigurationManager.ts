...
export class ConfigurationManager extends EventEmitter {
	...
	private watchingConfig: boolean = false;
	...
	public async updateConfiguration(
		...
		try {
			...
			// Track what changed
			result.appliedChanges = this.getConfigurationDiff(
				previousConfig,
				updatedConfig,
			);

			// Check if restart is required (for certain changes like file paths)
			result.requiresRestart = this.requiresRestart(
				previousConfig,
				updatedConfig,
			);
	...
	private async saveConfigurationFile(
		path: string,
		config: LoggerConfig,
	): Promise<void> {
	...
	private validateOutput(
		...
		if (
			!output.type ||
			!["console", "file", "external"].includes(output.type)
		) {
			result.errors.push({
	...
	private validateFileConfig(
		...
		if (
			config.maxFiles &&
			(!Number.isInteger(config.maxFiles) || config.maxFiles < 1)
		) {
			result.errors.push({
	...
	private getConfigurationDiff(
		...
		if (previous.level !== current.level) {
			changes.push(
				`Log level changed from ${previous.level} to ${current.level}`,
			);
		}

		if (previous.format !== current.format) {
			changes.push(
				`Log format changed from ${previous.format} to ${current.format}`,
			);
		}

		if (previous.includeStackTrace !== current.includeStackTrace) {
			changes.push(
				`Stack trace inclusion changed to ${current.includeStackTrace}`,
			);
		}

		if (previous.redactSensitiveData !== current.redactSensitiveData) {
			changes.push(
				`Sensitive data redaction changed to ${current.redactSensitiveData}`,
			);
		}
	...
	private requiresRestart(
		...
		return (
			JSON.stringify(previousFilePaths) !== JSON.stringify(currentFilePaths)
		);
	}
}
