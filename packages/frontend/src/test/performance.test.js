...
import { useDebounce, useThrottle, useRenderPerformance, } from '../hooks/usePerformanceOptimization';
...
describe('Frontend Performance Tests', () => {
    ...
    describe('Lazy Loading Performance', () => {
        it('should handle large datasets efficiently', async () => {
            ...
            const pageSize = 50;
            let loadCallCount = 0;
            const mockLoadFunction = vi.fn(async (page, size) => {
                loadCallCount++;
            ...
            const { result } = renderHook(() => useLazyLoading(mockLoadFunction, { pageSize, initialLoad: true }));
    ...
    describe('Bundle Size and Code Splitting', () => {
        ...
        it('should handle multiple concurrent chunk loads', async () => {
            const chunkCount = 5;
            const mockImports = Array.from({ length: chunkCount }, (_, i) => vi.fn().mockResolvedValue({
                default: () => `Lazy Component ${i}`,
            }));
            const startTime = performance.now();
            // Load multiple chunks concurrently
            const promises = mockImports.map((mockImport) => Promise.resolve(mockImport()).then((module) => module.default()));
