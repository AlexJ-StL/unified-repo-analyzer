...
describe("ErrorMessageService", () => {
	...
	describe("createTimeoutErrorMessage", () => {
		it("should create timeout error message", () => {
			const result = errorMessageService.createTimeoutErrorMessage(
				"C:\\slow\\path",
				5000,
			);
			...
			expect(result.suggestions).toContain(
				"Try increasing timeout beyond 5000ms",
			);
		});
	});
	...
	describe("getPlatformSpecificErrorGuidance", () => {
		...
		it("should return generic guidance for unknown error codes", () => {
			const result =
				errorMessageService.getPlatformSpecificErrorGuidance(
					"UNKNOWN_ERROR_CODE",
				);
	...
	describe("getCategoryColor", () => {
		it("should return correct colors for different categories", () => {
			...
			const permissionColor =
				errorMessageService.getCategoryColor("permission");
	...
	describe("Error message accuracy and clarity", () => {
		it("should provide clear and actionable error messages", () => {
			...
			testCases.forEach(({ code, expectedTitle, shouldContainSuggestion }) => {
				...
				expect(
					result.suggestions.some((s) => s.includes(shouldContainSuggestion)),
				).toBe(true);
				expect(result.suggestions.length).toBeGreaterThan(0);
		...
		it("should provide platform-appropriate suggestions", () => {
			...
			expect(
				windowsResult.suggestions.some((s) => s.includes("Windows Explorer")),
			).toBe(true);

			// Test macOS suggestions
			mockNavigator("MacIntel");
			const macResult = errorMessageService.createPathErrorMessage(errors);
			expect(macResult.suggestions.some((s) => s.includes("Finder"))).toBe(
				true,
			);
			...
			expect(linuxResult.suggestions.some((s) => s.includes("ls -la"))).toBe(
				true,
			);
		});
		...
		it("should provide contextual error messages based on path type", () => {
			...
			expect(uncResult.suggestions.some((s) => s.includes("server"))).toBe(
				true,
			);
		...
		it("should provide platform-specific guidance for new error codes", () => {
			...
			testCases.forEach((code) => {
				...
				const windowsGuidance =
					errorMessageService.getPlatformSpecificErrorGuidance(code);
		...
		it("should provide clear error categorization", () => {
			const categoryTests = [
				{ codes: ["PATH_NOT_FOUND", "PATH_TOO_LONG", "RESERVED_NAME"], category: "path" },
				{ codes: ["READ_PERMISSION_DENIED", "PERMISSION_CHECK_ERROR"], category: "permission" },
				{ codes: ["NETWORK_ERROR", "UNC_PATH_INVALID"], category: "network" },
				{ codes: ["TIMEOUT_ERROR", "FILESYSTEM_ERROR"], category: "system" },
				{ codes: ["INVALID_INPUT", "VALIDATION_ERROR"], category: "validation" },
			];
