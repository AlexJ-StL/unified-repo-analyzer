...
export class PerformanceService {
	...
	private handleNavigationEntry(entry: PerformanceNavigationTiming): void {
		...
		this.recordMetric(
			...
			entry.domInteractive - navigationStart,
		);
		this.recordMetric(
			"navigation.domComplete",
			entry.domComplete - navigationStart,
		);
	}
	...
	public getStats(): {
		...
		resources: {
			...
			byType: Record<
				string,
				{ count: number; totalSize: number; averageDuration: number }
			>;
		};
		interactions: {
			total: number;
		...
		const totalResourceSize = this.resourceTimings.reduce(
			(sum, r) => sum + (r.size || 0),
			0,
		);
		const averageResourceDuration =
			totalResources > 0
				? this.resourceTimings.reduce((sum, r) => sum + r.duration, 0) /
					totalResources
				: 0;

		// Interaction statistics
		const interactionMetrics = this.metrics.filter(
			(m) => m.tags?.type === "user-interaction",
		);
		...
		const apiErrorRate =
			totalApiCalls > 0 ? (apiErrors / totalApiCalls) * 100 : 0;
