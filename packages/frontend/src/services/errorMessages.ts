...
const ERROR_TEMPLATES: Record<string, Partial<UserFriendlyError>> = {
  ...
  NOT_DIRECTORY: {
    ...
    message: 'The path points to a file, but a directory is required for repository analysis.',
    suggestions: [
  ...
  UNC_PATH_INVALID: {
    ...
    message: 'Cannot access the network path. The network location may be unavailable.',
    suggestions: [
  ...
  NULL_BYTE_IN_PATH: {
    ...
    message: 'The path contains null bytes, which is a potential security vulnerability.',
    suggestions: [
  ...
  CONTROL_CHARACTERS: {
    ...
    message: 'The path contains control characters that are not allowed in file paths.',
    suggestions: [
  ...
  INVALID_COMPONENT_ENDING: {
    ...
    message: 'The path contains a folder or file name that ends with a space or dot.',
    suggestions: [
...
const WARNING_TEMPLATES: Record<string, Partial<UserFriendlyError>> = {
  ...
  CASE_SENSITIVITY_WARNING: {
    ...
    suggestions: ['Ensure the case matches exactly', 'This is more important on Unix-like systems'],
    severity: 'warning',
    category: 'path',
  },
...
class ErrorMessageService {
  ...
  createPathErrorMessage(
    ...
    const template = ERROR_TEMPLATES[primaryError.code] || ERROR_TEMPLATES.VALIDATION_ERROR;
  ...
  createWarningMessage(warnings: PathWarning[]): UserFriendlyError | null {
    ...
    return {
      ...
      message: primaryWarning.message || template.message || 'A warning occurred',
      details: warnings.length > 1 ? `${warnings.length} warnings found` : undefined,
   ...
   */
  createNetworkErrorMessage(path: string, errorMessage: string): UserFriendlyError {
    const isUNCPath = path.startsWith('\\\\');
    const template = isUNCPath ? ERROR_TEMPLATES.UNC_PATH_INVALID : ERROR_TEMPLATES.NETWORK_ERROR;
   ...
   */
  createTimeoutErrorMessage(path: string, timeoutMs: number): UserFriendlyError {
