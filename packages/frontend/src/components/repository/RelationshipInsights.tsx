...
const RelationshipInsightsComponent: React.FC<RelationshipInsightsProps> = ({ insights }) => {
  const getRelationshipStrength = () => {
    if (insights.totalRelationships === 0) return 0;
    return (insights.strongRelationships / insights.totalRelationships) * 100;
  };

  const getClusterDensity = () => {
    if (insights.totalRepositories === 0) return 0;
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{ width: `${getRelationshipStrength()}%` }}
              />
            </div>
            <p className="text-xs text-gray-500">
              {insights.strongRelationships} out of {insights.totalRelationships} relationships are strong (>70% similarity)
            </p>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{ width: `${Math.min(getClusterDensity(), 100)}%` }}
              />
            </div>
            <p className="text-xs text-gray-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div className="space-y-3">
            {insights.topLanguages.slice(0, 8).map((lang, index) => (
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div className="space-y-3">
            {insights.topFrameworks.slice(0, 8).map((framework, index) => (
              <div key={framework.framework} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-gray-900 capitalize">{framework.framework}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-16 bg-gray-200 rounded-full h-2">
                    <div
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {insights.architecturalPatterns.map((pattern) => (
        ...
        {insights.architecturalPatterns.length === 0 && (
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        ...
        <div className="space-y-3">
          {insights.integrationOpportunities.map((opportunity) => (
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                    opportunity.type === 'full-stack' ? 'bg-blue-100 text-blue-800' :
                    opportunity.type === 'microservices' ? 'bg-green-100 text-green-800' :
                    opportunity.type === 'library-ecosystem' ? 'bg-purple-100 text-purple-800' :
                    opportunity.type === 'mobile-backend' ? 'bg-pink-100 text-pink-800' :
              ...
              <div className="flex items-center gap-3 ml-4">
                <div className="text-right">
                  ...
                  <div className={`text-xs px-2 py-0.5 rounded-full ${
                    opportunity.estimatedEffort === 'low' ? 'bg-green-100 text-green-800' :
                    opportunity.estimatedEffort === 'medium' ? 'bg-yellow-100 text-yellow-800' :
        ...
        {insights.integrationOpportunities.length === 0 && (
          <p className="text-sm text-gray-500 text-center py-4">
