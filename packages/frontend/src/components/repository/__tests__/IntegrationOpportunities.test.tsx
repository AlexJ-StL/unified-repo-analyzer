...
describe("IntegrationOpportunities", () => {
  const mockOpportunities: IntegrationOpportunity[] = [
    ...
    },
    {
      ...
      description:
        "Integrate multiple services into microservices architecture",
      benefits: [
  ...
  it("should render all opportunities", () => {
    ...
    expect(
      screen.getByText("Full-Stack Application: Frontend + Backend")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Microservices Architecture: Service A + Service B")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Library Ecosystem: Utility Library")
    ).toBeInTheDocument();
  });
  ...
  it("should display opportunity details", () => {
    ...
    expect(
      screen.getByText("Integrate React frontend with Express backend")
    ).toBeInTheDocument();
  ...
  it("should filter opportunities by type", () => {
    ...
    expect(
      screen.queryByText("Microservices Architecture: Service A + Service B")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Library Ecosystem: Utility Library")
    ).not.toBeInTheDocument();
  });
  ...
  it("should expand opportunity details when clicked", () => {
    ...
    if (expandButton) {
      ...
      expect(
        screen.getByText("Unified development workflow")
      ).toBeInTheDocument();
      expect(screen.getByText("API design and versioning")).toBeInTheDocument();
      expect(
        screen.getByText("Design RESTful API endpoints")
      ).toBeInTheDocument();
    }
  });

  it("should call onSelectOpportunity when View Details is clicked", () => {
    // Use vi.fn() from Vitest for mocking
    const mockOnSelect = vi.fn();
    render(
      <IntegrationOpportunities
        opportunities={mockOpportunities}
        onSelectOpportunity={mockOnSelect}
      />
    );
  ...
  it("should sort opportunities by priority", () => {
    ...
    expect(titles[1]).toHaveTextContent(
      "Microservices Architecture: Service A + Service B"
    ); // Priority 70
  ...
  it("should show empty state when no opportunities", () => {
    ...
    expect(
      screen.getByText("No Integration Opportunities")
    ).toBeInTheDocument();
    expect(
      screen.getByText(
