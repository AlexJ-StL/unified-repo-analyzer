...
vi.mock('../SavedSearches', () => ({
    default: ({ onSelectSearch }) => (<div data-testid="saved-searches">
      <button onClick={() => onSelectSearch('test query')}>Select Search</button>
    </div>),
}));
vi.mock('../Pagination', () => ({
    default: ({ currentPage, totalPages, onPageChange, }) => (<div data-testid="pagination">
      <span>
...
vi.mock('../RepositoryCard', () => ({
    default: ({ repository, isSelected, onSelect, onView, }) => (<div data-testid={`repo-card-${repository.id}`}>
      <div>{repository.name}</div>
      <input type="checkbox" checked={isSelected} onChange={onSelect} data-testid={`select-${repository.id}`}/>
...
describe('SearchInterface', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        useRepositoryStore.mockReturnValue(mockStoreState);
    ...
    it('handles pagination correctly', async () => {
        ...
        expect(apiService.searchRepositories).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
            page: 2,
        }));
    });
    it('displays loading state correctly', async () => {
        useRepositoryStore.mockReturnValue({
            ...mockStoreState,
            isLoading: true,
        });
        render(<SearchInterface />);
    ...
    it('displays empty state correctly', async () => {
        ...
        render(<SearchInterface />);
        expect(screen.getByText('No repositories found matching your search criteria.')).toBeInTheDocument();
    });
});
//# sourceMappingURL=SearchInterface.test.js.map