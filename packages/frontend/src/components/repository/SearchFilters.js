...
const SearchFilters = () => {
    ...
    return (<div className="bg-white border border-gray-200 rounded-lg p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        {(filters.languages.length > 0 || filters.frameworks.length > 0) && (<button onClick={clearFilters} className="text-sm text-blue-600 hover:text-blue-800">
            Clear all
          </button>)}
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="space-y-2 max-h-40 overflow-y-auto">
          {availableLanguages.length === 0 ? (<p className="text-sm text-gray-500">No languages available</p>) : (availableLanguages.map((language) => (<div key={language} className="flex items-center">
                <input id={`language-${language}`} type="checkbox" checked={filters.languages.includes(language)} onChange={() => handleLanguageChange(language)} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
      ...
      <div>
        ...
        <div className="space-y-2 max-h-40 overflow-y-auto">
          {availableFrameworks.length === 0 ? (<p className="text-sm text-gray-500">No frameworks available</p>) : (availableFrameworks.map((framework) => (<div key={framework} className="flex items-center">
                <input id={`framework-${framework}`} type="checkbox" checked={filters.frameworks.includes(framework)} onChange={() => handleFrameworkChange(framework)} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
