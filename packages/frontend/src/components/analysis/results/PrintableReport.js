...
const PrintableReport = ({ analysis, batchAnalysis, className = '', }) => {
    ...
    if (analysis) {
        return (<div className={`print-report ${className}`}>
        <style jsx>{`
          @media print {
            .print-report {
        ...
        <div className="print-section">
          ...
          <ul className="print-list">
            {analysis.codeAnalysis.patterns.map((pattern, index) => (<li key={index}>
        ...
        <div className="print-section">
          ...
          <table className="print-table">
            ...
            {analysis.structure.keyFiles.slice(0, 15).map((file, index) => (<tr key={index}>
                <td>{file.path}</td>
                <td>{file.language}</td>
        ...
        <div className="print-section">
          ...
          <div className="print-grid">
            <div className="print-grid-item">
              ...
              {analysis.dependencies.production.length > 0 ? (<ul className="print-list">
                  {analysis.dependencies.production.map((dep, index) => (<li key={index}>
                      {dep.name}@{dep.version}
                    </li>))}
                </ul>) : (<p>None detected</p>)}
            </div>
            <div className="print-grid-item">
              <h3>Development Dependencies</h3>
              {analysis.dependencies.development.length > 0 ? (<ul className="print-list">
                  {analysis.dependencies.development.map((dep, index) => (<li key={index}>
                      {dep.name}@{dep.version}
                    </li>))}
                </ul>) : (<p>None detected</p>)}
            </div>
        ...
        <div className="print-section">
          ...
          <ul className="print-list">
            {analysis.insights.recommendations.map((rec, index) => (<li key={index}>{rec}</li>))}
          </ul>
        </div>
        ...
        <div className="print-section">
          ...
          <ul className="print-list">
            {analysis.insights.potentialIssues.map((issue, index) => (<li key={index}>{issue}</li>))}
          </ul>
        </div>
    ...
    if (batchAnalysis) {
        ...
        {batchAnalysis.repositories.map((repo, index) => (<div key={repo.id} className="print-section">
            <h2>
              {index + 1}. {repo.name}
            </h2>
            <div className="print-grid">
        ...
        {batchAnalysis.combinedInsights && (<div className="print-section">
            <h2>Combined Insights</h2>

            <h3>Commonalities</h3>
            <ul className="print-list">
              {batchAnalysis.combinedInsights.commonalities.map((item, index) => (<li key={index}>{item}</li>))}
            </ul>

            <h3>Differences</h3>
            <ul className="print-list">
              {batchAnalysis.combinedInsights.differences.map((item, index) => (<li key={index}>{item}</li>))}
            </ul>

            <h3>Integration Opportunities</h3>
            <ul className="print-list">
              {batchAnalysis.combinedInsights.integrationOpportunities.map((item, index) => (<li key={index}>{item}</li>))}
            </ul>
          </div>)}
      </div>);
    }
...
//# sourceMappingURL=PrintableReport.js.map