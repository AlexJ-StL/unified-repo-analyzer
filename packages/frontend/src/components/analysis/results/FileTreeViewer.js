...
const FileTreeViewer = ({ analysis }) => {
    ...
    const renderTreeNode = (node, depth = 0) => {
        ...
        return (<div key={node.path} className="select-none">
        <div className={`flex items-center py-1 px-2 hover:bg-gray-100 cursor-pointer ${selectedFile && selectedFile.path === node.path ? 'bg-blue-100' : ''}`} style={{ paddingLeft: `${depth * 16 + 8}px` }} onClick={() => node.isDirectory ? toggleNode(node.path) : node.fileInfo && selectFile(node.fileInfo)}>
          {node.isDirectory ? (<svg className={`h-4 w-4 mr-1 text-gray-500 transform ${isExpanded ? 'rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/>
            </svg>) : (<svg className="h-4 w-4 mr-1 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>)}
          <span className={`${node.isDirectory ? 'font-medium' : ''}`}>{node.name}</span>
          {node.fileInfo && (<span className="ml-2 text-xs text-gray-500">({node.fileInfo.language})</span>)}
        </div>
      ...
      <div className="md:col-span-2 border rounded-md overflow-hidden">
        ...
        <div className="p-4">
          {selectedFile ? (<div className="space-y-4">
              ...
              {selectedFile.description && (<div>
                  <h4 className="text-sm font-medium text-gray-700">Description</h4>
                  <p className="text-sm text-gray-600">{selectedFile.description}</p>
                </div>)}

              {selectedFile.useCase && (<div>
                  <h4 className="text-sm font-medium text-gray-700">Use Case</h4>
                  <p className="text-sm text-gray-600">{selectedFile.useCase}</p>
                </div>)}

              {selectedFile.functions && selectedFile.functions.length > 0 && (<div>
                  <h4 className="text-sm font-medium text-gray-700">Functions</h4>
                  <ul className="mt-1 space-y-1">
                    {selectedFile.functions.map((func, index) => (<li key={index} className="text-sm text-gray-600">
                        <span className="font-mono">{func.name}</span>
                        {func.description && (<span className="text-xs text-gray-500 ml-2">- {func.description}</span>)}
                      </li>))}
                  </ul>
                </div>)}

              {selectedFile.classes && selectedFile.classes.length > 0 && (<div>
                  <h4 className="text-sm font-medium text-gray-700">Classes</h4>
                  <ul className="mt-1 space-y-1">
                    {selectedFile.classes.map((cls, index) => (<li key={index} className="text-sm text-gray-600">
                        <span className="font-mono">{cls.name}</span>
                        {cls.description && (<span className="text-xs text-gray-500 ml-2">- {cls.description}</span>)}
                      </li>))}
                  </ul>
                </div>)}
