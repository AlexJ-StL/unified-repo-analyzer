...
const ExecutiveSummary = ({ analysis }) => {
    const { insights, name, language, languages, frameworks, fileCount, directoryCount, totalSize } = analysis;
    ...
    return (<div className="space-y-6">
      ...
      {insights.recommendations && insights.recommendations.length > 0 && (<div>
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Recommendations</h3>
          <ul className="list-disc pl-5 space-y-1">
            {insights.recommendations.map((recommendation, index) => (<li key={index} className="text-gray-700">
                {recommendation}
              </li>))}
          </ul>
        </div>)}

      {insights.potentialIssues && insights.potentialIssues.length > 0 && (<div>
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Potential Issues</h3>
          <ul className="list-disc pl-5 space-y-1">
            {insights.potentialIssues.map((issue, index) => (<li key={index} className="text-gray-700">
                {issue}
              </li>))}
          </ul>
        </div>)}

      {frameworks && frameworks.length > 0 && (<div>
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Frameworks & Technologies</h3>
          <div className="flex flex-wrap gap-2">
            {frameworks.map((framework, index) => (<span key={index} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                {framework}
              </span>))}
          </div>
        </div>)}
