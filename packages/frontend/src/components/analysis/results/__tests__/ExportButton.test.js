...
describe('ExportButton', () => {
    ...
    it('handles direct download correctly', async () => {
        ...
        apiService.exportAnalysis.mockResolvedValue(mockResponse);
        render(<ExportButton analysis={mockAnalysis}/>);
        const exportButton = screen.getByText('Export');
        fireEvent.click(exportButton);
        await waitFor(() => {
    ...
    it('shows loading state during export', async () => {
        ...
        apiService.exportAnalysis.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(mockResponse), 100)));
        render(<ExportButton analysis={mockAnalysis}/>);
        const exportButton = screen.getByText('Export');
        fireEvent.click(exportButton);
        await waitFor(() => {
    ...
    it('handles export errors gracefully', async () => {
        ...
        apiService.exportAnalysis.mockRejectedValue(mockError);
        // Mock window.alert
    ...
    it('works with batch analysis', async () => {
        ...
        apiService.exportBatchAnalysis.mockResolvedValue(mockResponse);
    ...
    it('falls back to clipboard when native share is not available', async () => {
        ...
        fireEvent.click(shareButtons[0]);
        await waitFor(() => {
            expect(navigator.clipboard.writeText).toHaveBeenCalledWith(`${window.location.origin}/api/export/download/test-export-id`);
...
//# sourceMappingURL=ExportButton.test.js.map