...
const ProgressTracker = ({ analysisId, className = '' }) => {
    ...
    const renderStatusContent = () => {
        switch (progress.status) {
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleCancel} className="px-3 py-1 bg-red-100 hover:bg-red-200 text-red-700 text-sm rounded-md transition-colors">
                Cancel
              </button>
            </div>

            <div className="w-full bg-gray-200 rounded-full h-2.5">
              <div className="bg-blue-600 h-2.5 rounded-full transition-all duration-300 ease-in-out" style={{ width: `${(progress.progress / progress.totalSteps) * 100}%` }}></div>
            </div>
          </div>);
            case 'completed':
                return (<div className="bg-green-50 border border-green-200 rounded-md p-4">
            <div className="flex">
    ...
    return (<div className={`progress-tracker ${className}`}>
      <div className="mb-4">
        <div className="flex items-center justify-between">
      ...
      {/* Log display */}
      {logs.length > 0 && (<div className="mt-6">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-md p-3 max-h-40 overflow-y-auto">
            <ul className="space-y-1 text-xs font-mono">
              {logs.map((log, index) => (<li key={index} className="text-gray-700">
