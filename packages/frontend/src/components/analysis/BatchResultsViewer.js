...
const BatchResultsViewer = ({ batchResult }) => {
    ...
    return (<div className="bg-white shadow rounded-lg overflow-hidden">
      <div className="border-b border-gray-200">
        <div className="flex items-center justify-between px-6 py-4">
          <nav className="flex -mb-px overflow-x-auto" aria-label="Tabs">
            {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`whitespace-nowrap py-2 px-4 border-b-2 font-medium text-sm mr-6 ${activeTab === tab.id
                ? 'border-blue-500 text-blue-600'
                : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`} aria-current={activeTab === tab.id ? 'page' : undefined}>
                {tab.label}
              </button>))}
          </nav>

          <div className="flex items-center space-x-3">
            <button onClick={handlePrint} className="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
      ...
      <div className="p-6">
        ...
        {activeTab === 'repositories' && (<div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900">Repository Results</h3>
            <div className="grid gap-6">
              {batchResult.repositories.map((repo, index) => (<div key={repo.id} className="border border-gray-200 rounded-lg p-6">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <div className={`text-sm font-medium ${repo.codeAnalysis.complexity.codeQuality === 'excellent'
                    ? 'text-green-600'
        ...
        {activeTab === 'insights' && (<div className="space-y-6">
            ...
            {batchResult.combinedInsights ? (<div className="space-y-6">
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-3">Commonalities</h4>
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <ul className="space-y-2">
                      {batchResult.combinedInsights.commonalities.map((item, index) => (<li key={index} className="text-sm text-blue-800 flex items-start">
                ...
                <div>
                  ...
                  <div className="bg-yellow-50 p-4 rounded-lg">
                    <ul className="space-y-2">
                      {batchResult.combinedInsights.differences.map((item, index) => (<li key={index} className="text-sm text-yellow-800 flex items-start">
                ...
                <div>
                  ...
                  <div className="bg-green-50 p-4 rounded-lg">
                    <ul className="space-y-2">
                      {batchResult.combinedInsights.integrationOpportunities.map((item, index) => (<li key={index} className="text-sm text-green-800 flex items-start">
...
//# sourceMappingURL=BatchResultsViewer.js.map