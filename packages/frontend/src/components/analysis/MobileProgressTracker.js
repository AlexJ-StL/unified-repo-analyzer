...
const MobileProgressTracker = ({ analysisId, className = '', }) => {
    ...
    const renderStatusContent = () => {
        switch (progress.status) {
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleCancel} className="px-2 py-1 bg-red-100 hover:bg-red-200 text-red-700 text-xs rounded-md transition-colors">
                Cancel
              </button>
            </div>

            <div className="w-full bg-gray-200 rounded-full h-2">
              <div className="bg-blue-600 h-2 rounded-full transition-all duration-300 ease-in-out" style={{ width: `${(progress.progress / progress.totalSteps) * 100}%` }}></div>
            </div>
          </div>);
            case 'completed':
                return (<div className="bg-green-50 border border-green-200 rounded-md p-3">
            <div className="flex">
            ...
            case 'failed':
                return (<div className="bg-red-50 border border-red-200 rounded-md p-3">
            <div className="flex">
              <div className="flex-shrink-0">
    ...
    return (<div className={`mobile-progress-tracker ${className}`}>
      <div className="mb-2">
        <div className="flex items-center justify-between">
      ...
      {/* Log display toggle */}
      {logs.length > 0 && (<div className="mt-3">
          <button onClick={() => setShowLogs(!showLogs)} className="text-xs text-blue-600 flex items-center">
            {showLogs ? 'Hide logs' : 'Show logs'}
            <svg className={`ml-1 h-3 w-3 transform ${showLogs ? 'rotate-180' : ''}`} fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd"/>
            </svg>
          </button>

          {showLogs && (<div className="mt-2 bg-gray-50 border border-gray-200 rounded-md p-2 max-h-24 overflow-y-auto">
              <ul className="space-y-1 text-xs font-mono">
                {logs.slice(-5).map((log, index) => (<li key={index} className="text-gray-700 truncate">
...
//# sourceMappingURL=MobileProgressTracker.js.map