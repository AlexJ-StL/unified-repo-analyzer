...
describe('EnhancedErrorDisplay', () => {
  ...
  it('should render learn more link when provided', () => {
    ...
    expect(learnMoreLink.closest('a')).toHaveAttribute('href', 'https://example.com/help');
  ...
  it('should show platform-specific tips when expanded', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Use backslashes or forward slashes')).toBeInTheDocument();
    });
  ...
  it('should handle retry callback', () => {
    ...
    render(<EnhancedErrorDisplay userFriendlyError={mockUserFriendlyError} onRetry={onRetry} />);
  ...
  it('should handle dismiss callback', () => {
    ...
    render(
      <EnhancedErrorDisplay userFriendlyError={mockUserFriendlyError} onDismiss={onDismiss} />
    );
  ...
  it('should render in compact mode', () => {
    render(<EnhancedErrorDisplay userFriendlyError={mockUserFriendlyError} compact={true} />);
  ...
  it('should not render when dismissed', () => {
    render(<EnhancedErrorDisplay userFriendlyError={mockUserFriendlyError} onDismiss={vi.fn()} />);
  ...
  it('should render warning message when no errors but warnings exist', () => {
    ...
    expect(errorMessageService.createWarningMessage).toHaveBeenCalledWith(mockPathWarnings);
  });

  it('should not render when no errors, warnings, or user-friendly error', () => {
    const { container } = render(<EnhancedErrorDisplay errors={[]} warnings={[]} />);
  ...
  it('should hide suggestions when showSuggestions is false', () => {
    render(
      <EnhancedErrorDisplay userFriendlyError={mockUserFriendlyError} showSuggestions={false} />
    );
  ...
  it('should create error message from path errors when no user-friendly error provided', () => {
    const { errorMessageService } = require('../../../services/errorMessages');
    errorMessageService.createPathErrorMessage.mockReturnValue(mockUserFriendlyError);
  ...
  describe('Error message quality', () => {
    it('should display clear and actionable error messages', () => {
      const testError: UserFriendlyError = {
        ...
        message: 'The specified repository path does not exist or cannot be accessed.',
        suggestions: [
      ...
      expect(
        screen.getByText('The specified repository path does not exist or cannot be accessed.')
      ).toBeInTheDocument();

      // Suggestions should be actionable
      expect(
        screen.getByText('Double-check the spelling and capitalization of the path')
      ).toBeInTheDocument();
      expect(screen.getByText('Copy the path directly from your file manager')).toBeInTheDocument();
    });
    ...
    it('should provide contextual help for different error categories', () => {
      ...
      expect(screen.getByText('Show platform-specific tips')).toBeInTheDocument();
    });
  });
});
