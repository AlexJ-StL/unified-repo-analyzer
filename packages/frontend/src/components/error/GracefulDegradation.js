...
const GracefulDegradation = ({ children, fallback, feature, isEnabled, error, onRetry, showWarning = true, warningMessage, }) => {
    ...
    // Default fallback UI
    const defaultMessage = warningMessage ||
        (error
    ...
    return (<div className="bg-yellow-50 border border-yellow-200 rounded-md p-4">
      <div className="flex">
        <div className="flex-shrink-0">
          {error ? (<ExclamationTriangleIcon className="h-5 w-5 text-yellow-400"/>) : (<EyeSlashIcon className="h-5 w-5 text-yellow-400"/>)}
        </div>
        <div className="ml-3">
          <h3 className="text-sm font-medium text-yellow-800">Feature Unavailable</h3>
          <div className="mt-2 text-sm text-yellow-700">
            <p>{defaultMessage}</p>
            {error && process.env.NODE_ENV === 'development' && (<details className="mt-2">
          ...
          {onRetry && (<div className="mt-3">
              <button onClick={onRetry} className="text-sm font-medium text-yellow-800 underline hover:no-underline">
