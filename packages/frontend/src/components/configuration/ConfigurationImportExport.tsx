...
const ConfigurationImportExport: React.FC = () => {
	const { exportConfiguration, importConfiguration, createBackup } =
		useSettingsStore();
	...
	const handleExport = async () => {
		...
		try {
			...
			showToast({ type: "error", title: "Failed to export configuration" });
		} finally {
	...
	const handleImport = async () => {
		if (!importData.trim()) {
			showToast({ type: "error", title: "Please paste configuration data" });
			return;
		}
		...
		try {
			...
			showToast({ type: "error", title: "Failed to import configuration" });
		} finally {
					...
					<div className="ml-3">
						...
						<div className="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
							<ul className="list-disc list-inside space-y-1">
								...
								<li>
									Keep exported files secure and don't share them publicly
								</li>
							</ul>
						</div>
					</div>
