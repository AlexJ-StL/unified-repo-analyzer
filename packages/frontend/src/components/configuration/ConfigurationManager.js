...
import { CogIcon, UserIcon, ServerIcon, DocumentArrowDownIcon, FolderIcon, RectangleStackIcon, PaintBrushIcon, } from '@heroicons/react/24/outline';
...
const ConfigurationManager = () => {
    ...
    const { isLoading, error, loadPreferences, loadWorkspaces, loadProjects, loadProfiles, loadPresets, clearError, } = useSettingsStore();
    const { showToast } = useToast();
    const tabs = [
        { name: 'General', icon: UserIcon, component: GeneralPreferences },
        { name: 'Analysis', icon: CogIcon, component: AnalysisPreferences },
        { name: 'LLM Providers', icon: ServerIcon, component: LLMProviderPreferences },
        { name: 'Export', icon: DocumentArrowDownIcon, component: ExportPreferences },
        ...
        { name: 'Import/Export', icon: DocumentArrowDownIcon, component: ConfigurationImportExport },
    ];
    useEffect(() => {
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-64 flex-shrink-0">
            <Tab.List className="flex flex-col space-y-1 bg-white dark:bg-gray-800 rounded-lg shadow p-2">
              {tabs.map((tab) => (<Tab key={tab.name} className={({ selected }) => classNames('flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors', selected
          ...
          <div className="flex-1">
            <Tab.Panels>
              {tabs.map((tab, index) => (<Tab.Panel key={index} className="focus:outline-none">
