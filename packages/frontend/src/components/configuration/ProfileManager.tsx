...
const ProfileManager: React.FC = () => {
	...
	const handleApplyProfile = async (profileId: string, profileName: string) => {
		try {
			await applyProfile(profileId);
			showToast({
				type: "success",
				title: `Profile "${profileName}" applied successfully`,
			});
		} catch {
			showToast({ type: "error", title: "Failed to apply profile" });
		}
	};
