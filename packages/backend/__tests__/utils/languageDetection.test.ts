...
describe("Language Detection Utilities", () => {
	...
	beforeEach(async () => {
		...
		await writeFile(
			path.join(testDir, "script.sh"),
			'#!/bin/bash\necho "Hello"',
		);
		await writeFile(
			path.join(testDir, "script.rb"),
			'#!/usr/bin/env ruby\nputs "Hello"',
		);
		await writeFile(
	...
	describe("detectLanguageFromShebang", () => {
		test("should detect language based on shebang", () => {
			...
			expect(
				detectLanguageFromShebang('#!/usr/bin/env python\nprint("Hello")'),
			).toBe("Python");
			expect(
				detectLanguageFromShebang('#!/usr/bin/env ruby\nputs "Hello"'),
			).toBe("Ruby");
			expect(
				detectLanguageFromShebang('#!/usr/bin/env node\nconsole.log("Hello")'),
			).toBe(null);
		});
	...
	describe("detectLanguage", () => {
		...
		test("should use provided content if available", async () => {
			expect(
				await detectLanguage("unknown.file", '#!/bin/bash\necho "Hello"'),
			).toBe("Shell");
			expect(
			...
			expect(
				await detectLanguage("unknown.file", 'console.log("Hello");'),
			).toBe("Unknown");
		});
