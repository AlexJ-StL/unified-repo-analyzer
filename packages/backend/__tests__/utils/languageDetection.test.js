...
import { detectLanguageFromPath, detectLanguageFromShebang, detectLanguage, detectFrameworks, LANGUAGES, } from '../../src/utils/languageDetection';
...
describe('Language Detection Utilities', () => {
    ...
    beforeEach(async () => {
        ...
        await writeFile(path.join(testDir, 'component.tsx'), 'export const Component = () => <div>Hello</div>;');
        ...
        await writeFile(path.join(testDir, 'package.json'), '{"name":"test","dependencies":{"react":"17.0.2"}}');
    ...
    describe('detectFrameworks', () => {
        test('should detect frameworks based on file patterns', async () => {
            ...
            await writeFile(path.join(testDir, 'react-project', 'package.json'), '{"dependencies":{"react":"17.0.2","react-dom":"17.0.2"}}');
            await writeFile(path.join(testDir, 'react-project', 'App.jsx'), 'import React from "react"; export default () => <div>App</div>;');
            const reactFiles = [
                path.join(testDir, 'react-project', 'package.json'),
                path.join(testDir, 'react-project', 'App.jsx'),
            ];
            const reactFrameworks = await detectFrameworks(reactFiles, path.join(testDir, 'react-project', 'package.json'));
        ...
        test('should return empty array for unrecognized projects', async () => {
            ...
            await writeFile(path.join(testDir, 'generic-project', 'package.json'), '{"dependencies":{"lodash":"4.17.21"}}');
            ...
            const genericFrameworks = await detectFrameworks(genericFiles, path.join(testDir, 'generic-project', 'package.json'));
