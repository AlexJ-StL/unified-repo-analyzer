...
import { calculateFileImportance, calculateImportanceFactors, sortFilesByImportance, IMPORTANT_FILE_PATTERNS, IMPORTANT_EXTENSIONS, } from '../../src/utils/fileImportance';
describe('File Importance Utilities', () => {
    ...
    describe('calculateImportanceFactors', () => {
        test('should calculate importance factors for a file', () => {
            ...
            const factors = calculateImportanceFactors(filePath, fileSize, mockRepoPath);
        ...
        test('should give higher name score to important configuration files', () => {
            ...
            const fileSize = 1024;
            const configFactors = calculateImportanceFactors(configFilePath, fileSize, mockRepoPath);
            const regularFactors = calculateImportanceFactors(regularFilePath, fileSize, mockRepoPath);
        ...
        test('should give higher name score to entry point files', () => {
            ...
            const fileSize = 1024;
            const entryFactors = calculateImportanceFactors(entryFilePath, fileSize, mockRepoPath);
            const regularFactors = calculateImportanceFactors(regularFilePath, fileSize, mockRepoPath);
        ...
        test('should give higher location score to files at the root', () => {
            ...
            const fileSize = 1024;
            const rootFactors = calculateImportanceFactors(rootFilePath, fileSize, mockRepoPath);
            const deepFactors = calculateImportanceFactors(deepFilePath, fileSize, mockRepoPath);
        ...
        test('should give higher location score to files in core directories', () => {
            ...
            const fileSize = 1024;
            const coreFactors = calculateImportanceFactors(coreFilePath, fileSize, mockRepoPath);
            const nonCoreFactors = calculateImportanceFactors(nonCoreFilePath, fileSize, mockRepoPath);
        ...
        test('should give optimal size score to medium-sized files', () => {
            ...
            const tinyFactors = calculateImportanceFactors(tinyFilePath, tinySize, mockRepoPath);
            const mediumFactors = calculateImportanceFactors(mediumFilePath, mediumSize, mockRepoPath);
            const hugeFactors = calculateImportanceFactors(hugeFilePath, hugeSize, mockRepoPath);
    ...
    describe('calculateFileImportance', () => {
        ...
        test('should give higher scores to important files', () => {
            ...
            const fileSize = 1024;
            const importantScore = calculateFileImportance(importantFilePath, fileSize, mockRepoPath);
            const lessImportantScore = calculateFileImportance(lessImportantFilePath, fileSize, mockRepoPath);
    ...
    describe('sortFilesByImportance', () => {
        test('should sort files by importance score in descending order', () => {
            ...
            for (let i = 0; i < sortedFiles.length - 1; i++) {
                expect(sortedFiles[i].importance).toBeGreaterThanOrEqual(sortedFiles[i + 1].importance);
            }
        });
...
//# sourceMappingURL=fileImportance.test.js.map