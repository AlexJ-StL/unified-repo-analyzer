 ...
 * @param res - Express response
 */
export const getHomeDirectory = async (_req: Request, res: Response): Promise<void> => {
  try {
 ...
 * @param res - Express response
 */
export const validateDirectory = async (req: Request, res: Response): Promise<void> => {
  const requestId = (req.headers['x-request-id'] as string) || `req-${Date.now()}`;
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error(`Directory validation failed: ${errorMessage}. RequestId: ${requestId}`);
 ...
 * @param res - Express response
 */
export const getRecentRepositories = async (_req: Request, res: Response): Promise<void> => {
  try {
