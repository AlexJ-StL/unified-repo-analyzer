...
describe('Batch Analysis API', () => {
    ...
    beforeEach(() => {
        ...
        // Mock AnalysisEngine
        AnalysisEngine.prototype.analyzeMultipleRepositoriesWithQueue.mockImplementation(async (paths, options, concurrency, progressCallback) => {
    ...
    test('should analyze multiple repositories', async () => {
        ...
        expect(AnalysisEngine.prototype.analyzeMultipleRepositoriesWithQueue).toHaveBeenCalledWith(['/path/to/repo1', '/path/to/repo2', '/path/to/repo3'], expect.objectContaining({
            mode: 'quick',
            maxFiles: 10,
            maxLinesPerFile: 100,
            includeLLMAnalysis: false,
    ...
    test('should handle errors during batch analysis', async () => {
        ...
        AnalysisEngine.prototype.analyzeMultipleRepositoriesWithQueue.mockRejectedValueOnce(new Error('Batch analysis failed'));
        const response = await request(app)
            .post('/api/analyze/batch')
            .send({
...
//# sourceMappingURL=batch-analyze.test.js.map