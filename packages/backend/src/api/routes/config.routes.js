 ...
 */
router.put('/preferences', body('general').optional().isObject(), body('analysis').optional().isObject(), body('llmProvider').optional().isObject(), body('export').optional().isObject(), body('ui').optional().isObject(), async (req, res) => {
    try {
 ...
 */
router.patch('/preferences/:section', param('section').isIn(['general', 'analysis', 'llmProvider', 'export', 'ui']), async (req, res) => {
    try {
        const { section } = req.params;
        const preferences = await configurationService.updatePreferences(section, req.body);
        res.json(preferences);
    }
    catch (error) {
 ...
 */
router.post('/workspaces', body('name').isString().notEmpty(), body('path').isString().notEmpty(), body('preferences').optional().isObject(), async (req, res) => {
    try {
 ...
 */
router.put('/workspaces/:id', param('id').isUUID(), body('name').optional().isString().notEmpty(), body('path').optional().isString().notEmpty(), body('preferences').optional().isObject(), async (req, res) => {
    try {
        const workspace = await configurationService.updateWorkspaceConfiguration(req.params.id, req.body);
        res.json(workspace);
    }
    catch (error) {
 ...
 */
router.post('/projects', body('name').isString().notEmpty(), body('path').isString().notEmpty(), body('workspaceId').optional().isUUID(), body('preferences').optional().isObject(), body('customIgnorePatterns').optional().isArray(), body('customAnalysisOptions').optional().isObject(), async (req, res) => {
    try {
 ...
 */
router.post('/profiles', body('name').isString().notEmpty(), body('description').isString(), body('preferences').isObject(), body('isDefault').optional().isBoolean(), async (req, res) => {
    try {
 ...
 */
router.get('/effective-preferences', query('projectPath').isString().notEmpty(), async (req, res) => {
    try {
        const preferences = await configurationService.getEffectivePreferences(req.query.projectPath);
        res.json(preferences);
    }
    catch (error) {
