...
export class RelationshipService {
     ...
     */
    async generateRelationshipGraph(repositoryIds) {
        ...
        const edges = index.relationships
            .filter((rel) => repositories.some((repo) => repo.id === rel.sourceId) &&
     ...
     */
    async analyzeIntegrationOpportunities(repositoryIds) {
        const index = this.indexSystem.getIndex();
     ...
     */
    async generateRelationshipInsights(repositoryIds) {
        const index = this.indexSystem.getIndex();
        const repositories = repositoryIds
            ? index.repositories.filter((repo) => repositoryIds.includes(repo.id))
            : index.repositories;
        const relationships = index.relationships.filter((rel) => repositories.some((repo) => repo.id === rel.sourceId) &&
            repositories.some((repo) => repo.id === rel.targetId));
        ...
        const integrationOpportunities = await this.analyzeIntegrationOpportunities(repositories.map((repo) => repo.id));
     ...
     */
    determineRepositoryType(repo) {
        // Check for frontend
        if (repo.frameworks.some((fw) => ['react', 'vue', 'angular', 'svelte'].includes(fw.toLowerCase()))) {
            return 'frontend';
        }
        // Check for backend
        if (repo.frameworks.some((fw) => ['express', 'nest', 'django', 'flask', 'spring'].includes(fw.toLowerCase()))) {
            return 'backend';
        }
        // Check for mobile
     ...
     */
    async generateClusters(repositories, relationships) {
        const clusters = [];
     ...
     */
    async analyzeFullStackOpportunities(repositories) {
        const opportunities = [];
        const frontendRepos = repositories.filter((repo) => repo.frameworks.some((fw) => ['react', 'vue', 'angular'].includes(fw.toLowerCase())));
        const backendRepos = repositories.filter((repo) => repo.frameworks.some((fw) => ['express', 'nest', 'django', 'flask'].includes(fw.toLowerCase())));
        for (const frontend of frontendRepos) {
            for (const backend of backendRepos) {
                if (frontend.id !== backend.id) {
                    const sharedLanguages = frontend.languages.filter((lang) => backend.languages.includes(lang));
     ...
     */
    async analyzeMicroservicesOpportunities(repositories) {
        const opportunities = [];
        const serviceRepos = repositories.filter((repo) => repo.name.toLowerCase().includes('service') ||
            repo.tags.some((tag) => tag.toLowerCase().includes('service')));
        if (serviceRepos.length >= 2) {
            const combinations = this.generateCombinations(serviceRepos, Math.min(serviceRepos.length, 4));
     ...
     */
    async analyzeLibraryEcosystemOpportunities(repositories) {
        const opportunities = [];
        const libraryRepos = repositories.filter((repo) => repo.name.toLowerCase().includes('lib') ||
            repo.name.toLowerCase().includes('util') ||
            repo.tags.some((tag) => tag.toLowerCase().includes('library')));
        const applicationRepos = repositories.filter((repo) => !libraryRepos.includes(repo) && (repo.frameworks.length > 0 || repo.size > 1000000));
        if (libraryRepos.length > 0 && applicationRepos.length > 0) {
            for (const app of applicationRepos) {
                const compatibleLibraries = libraryRepos.filter((lib) => lib.languages.some((lang) => app.languages.includes(lang)));
     ...
     */
    async analyzeMobileBackendOpportunities(repositories) {
        const opportunities = [];
        const mobileRepos = repositories.filter((repo) => repo.languages.some((lang) => ['swift', 'kotlin', 'dart'].includes(lang.toLowerCase())) ||
            repo.frameworks.some((fw) => ['react-native', 'flutter', 'ionic'].includes(fw.toLowerCase())));
        const backendRepos = repositories.filter((repo) => repo.frameworks.some((fw) => ['express', 'nest', 'django', 'flask'].includes(fw.toLowerCase())));
     ...
     */
    async analyzeToolChainOpportunities(repositories) {
        const opportunities = [];
        const toolRepos = repositories.filter((repo) => repo.name.toLowerCase().includes('tool') ||
     ...
     */
    analyzeArchitecturalPatterns(repositories) {
        const patternCount = new Map();
        repositories.forEach((repo) => {
            // Detect patterns from frameworks and languages
            if (repo.frameworks.some((fw) => ['react', 'vue', 'angular'].includes(fw.toLowerCase()))) {
                patternCount.set('SPA (Single Page Application)', (patternCount.get('SPA (Single Page Application)') || 0) + 1);
            }
            ...
            if (repo.frameworks.some((fw) => ['next.js', 'nuxt'].includes(fw.toLowerCase()))) {
                patternCount.set('Server-Side Rendering', (patternCount.get('Server-Side Rendering') || 0) + 1);
            }
            if (repo.name.toLowerCase().includes('service') ||
            ...
            if (repo.frameworks.some((fw) => ['django', 'flask', 'spring'].includes(fw.toLowerCase()))) {
                patternCount.set('MVC (Model-View-Controller)', (patternCount.get('MVC (Model-View-Controller)') || 0) + 1);
            }
        });
     ...
     */
    findStronglyConnectedComponents(repositories, relationships) {
        // Simple connected components algorithm
        const visited = new Set();
        const components = [];
        const dfs = (repoId, component) => {
            ...
            if (repo) {
                ...
                const connectedRelationships = relationships.filter((rel) => (rel.sourceId === repoId || rel.targetId === repoId) && rel.strength > 0.5);
     ...
     */
    generateCombinations(items, maxSize) {
        const combinations = [];
        for (let size = 2; size <= Math.min(items.length, maxSize); size++) {
            const generateCombinationsOfSize = (start, current) => {
