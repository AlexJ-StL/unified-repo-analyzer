...
describe('ConfigurationService', () => {
    ...
    describe('initialize', () => {
        it('should create configuration directory and default files', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith(path.join(mockConfigDir, 'user-preferences.json'), JSON.stringify(DEFAULT_USER_PREFERENCES, null, 2));
        });
        it('should not overwrite existing files', async () => {
            mockFs.mkdir.mockResolvedValue(undefined);
            mockFs.access.mockResolvedValue(undefined); // Files exist
            await configService.initialize();
            expect(mockFs.writeFile).not.toHaveBeenCalledWith(expect.stringContaining('user-preferences.json'), expect.any(String));
        });
    });
    describe('getUserPreferences', () => {
        it('should return user preferences from file', async () => {
            ...
            expect(mockFs.readFile).toHaveBeenCalledWith(path.join(mockConfigDir, 'user-preferences.json'), 'utf-8');
        });
    ...
    describe('workspace management', () => {
        ...
        it('should delete workspace configuration', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith(expect.stringContaining('workspaces.json'), JSON.stringify([], null, 2));
        });
    });
    describe('backup and restore', () => {
        it('should create configuration backup', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith(expect.stringContaining(`backup-${backup.id}.json`), expect.any(String));
        });
    ...
    describe('import and export', () => {
        ...
        it('should import configuration', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith(expect.stringContaining('user-preferences.json'), JSON.stringify(DEFAULT_USER_PREFERENCES, null, 2));
        });
    });
});
