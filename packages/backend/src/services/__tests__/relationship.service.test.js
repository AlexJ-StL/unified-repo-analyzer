...
describe('RelationshipService', () => {
    ...
    let mockIndexSystem;
    beforeEach(() => {
        mockIndexSystem = new IndexSystem();
    ...
    describe('analyzeIntegrationOpportunities', () => {
        ...
        it('should identify microservices opportunities', async () => {
            ...
            expect(microservicesOpportunity.benefits).toContain('Scalable and maintainable architecture');
        });
    ...
    describe('generateRelationshipInsights', () => {
        it('should generate comprehensive relationship insights', async () => {
            ...
            expect(insights.topLanguages).toContainEqual({ language: 'javascript', count: 2 });
            expect(insights.topFrameworks).toContainEqual({ framework: 'react', count: 1 });
        ...
        it('should filter insights by repository IDs when provided', async () => {
            ...
            expect(insights.topLanguages).toContainEqual({ language: 'javascript', count: 2 });
            expect(insights.topLanguages).not.toContainEqual({ language: 'python', count: 1 });
        });
        it('should calculate architectural patterns correctly', async () => {
            ...
            const insights = await relationshipService.generateRelationshipInsights();
            const spaPattern = insights.architecturalPatterns.find((p) => p.pattern === 'SPA (Single Page Application)');
...
function createMockIndexedRepository(id, name, languages, frameworks) {
    return {
        id,
        name,
        path: `/path/to/${id}`,
...
//# sourceMappingURL=relationship.service.test.js.map