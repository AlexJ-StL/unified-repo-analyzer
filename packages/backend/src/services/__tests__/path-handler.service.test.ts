...
describe('PathHandler Service', () => {
  ...
  describe('Path Normalization - Cross Platform', () => {
    ...
    it('should handle empty or invalid paths', () => {
      ...
      expect(() => pathHandler.normalizePath(null as any)).toThrow('Path must be a non-empty string');
      expect(() => pathHandler.normalizePath(undefined as any)).toThrow('Path must be a non-empty string');
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should provide helpful error messages', async () => {
      ...
      const errorWithSuggestions = result.errors.find(e => e.suggestions && e.suggestions.length > 0);
