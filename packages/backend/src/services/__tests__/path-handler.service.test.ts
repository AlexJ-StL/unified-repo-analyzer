...
describe("PathHandler Service", () => {
  ...
  describe("Path Format Validation", () => {
    ...
    it("should detect trailing spaces and dots", async () => {
      ...
      expect(
        spaceResult.errors.some((e) => e.code === "INVALID_COMPONENT_ENDING")
      ).toBe(true);

      const dotResult = await handler.validatePath("C:\\Users\\test.\\file");
      expect(dotResult.isValid).toBe(false);
      expect(
        dotResult.errors.some((e) => e.code === "INVALID_COMPONENT_ENDING")
      ).toBe(true);
    });
    ...
    it("should validate drive letter format", async () => {
      ...
      const driveErrors = result.errors.filter(
    ...
    it("should detect invalid drive letter format", async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some((e) => e.code === "INVALID_DRIVE_LETTER")).toBe(
        true
      );
    });
    ...
    it("should validate UNC path format", async () => {
      ...
      // Valid UNC path
      const validResult = await handler.validatePath(
        "\\\\server\\share\\folder"
      );
      const uncErrors = validResult.errors.filter(
      ...
      expect(
        invalidResult.errors.some((e) => e.code === "INVALID_UNC_PATH")
      ).toBe(true);
    });
    ...
    it("should warn about long path components", async () => {
      ...
      const result = await handler.validatePath(
        `C:\\Users\\${longComponent}\\test`
      );

      expect(result.warnings.some((w) => w.code === "COMPONENT_TOO_LONG")).toBe(
        true
      );
    });
  ...
  describe("Permission Checking", () => {
    ...
    it("should detect Windows system paths", async () => {
      ...
      const handler = new PathHandler("win32");
      const result = await handler.checkPermissions(
        "C:\\Windows\\System32\\test"
      );

      expect(result.errors.some((e) => e.code === "SYSTEM_PATH_ACCESS")).toBe(
        true
      );
    });
    ...
    it("should provide detailed error messages for Windows permission issues", async () => {
      ...
      expect(
        result.errors.some((e) => e.code === "READ_PERMISSION_DENIED")
      ).toBe(true);
      expect(
        result.errors.some((e) => e.code === "WRITE_PERMISSION_DENIED")
      ).toBe(true);
    });
  ...
  describe("Relative Path Resolution", () => {
    it("should resolve relative paths with base path", () => {
      const result = pathHandler.resolveRelativePath(
        "./documents",
        "/home/user"
      );
      expect(result).toContain("documents");
  ...
  describe("Timeout and Progress Handling", () => {
    ...
    it("should timeout long-running operations", async () => {
      ...
      try {
        ...
        expect(result.errors.some((e) => e.code === "TIMEOUT_ERROR")).toBe(
          true
        );
      } catch (error) {
