...
// Mock the config service to avoid singleton initialization issues
const mockHomeDir = '/mock/home';

vi.mock('../config.service', () => {
  return {
    ConfigurationService: vi.fn().mockImplementation(() => ({
      configDir: path.join(mockHomeDir, '.repo-analyzer'),
      userConfigPath: path.join(mockHomeDir, '.repo-analyzer', 'user-preferences.json'),
      workspacesConfigPath: path.join(mockHomeDir, '.repo-analyzer', 'workspaces.json'),
      projectsConfigPath: path.join(mockHomeDir, '.repo-analyzer', 'projects.json'),
      profilesConfigPath: path.join(mockHomeDir, '.repo-analyzer', 'profiles.json'),
      backupsDir: path.join(mockHomeDir, '.repo-analyzer', 'backups'),
      ...
      saveWorkspaceConfiguration: vi.fn().mockResolvedValue({
      ...
      updateWorkspaceConfiguration: vi.fn().mockResolvedValue({
        id: 'mock-workspace-id',
      ...
      deleteWorkspaceConfiguration: vi.fn().mockResolvedValue(undefined),
      ...
      importConfiguration: vi.fn().mockResolvedValue(undefined),
      validateUserPreferences: vi.fn().mockReturnValue({
        isValid: true,
        errors: [],
      }),
    })),
