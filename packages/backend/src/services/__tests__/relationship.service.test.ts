...
describe("RelationshipService", () => {
	...
	let mockIndexSystem: anyed<IndexSystem>;

	beforeEach(() => {
		mockIndexSystem = new IndexSystem() as anyed<IndexSystem>;
	...
	describe("analyzeIntegrationOpportunities", () => {
		it("should identify full-stack opportunities", async () => {
			...
			const opportunities =
				await relationshipService.analyzeIntegrationOpportunities([
					"frontend",
					"backend",
				]);
			...
			const fullStackOpportunity = opportunities.find(
				(opp) => opp.type === "full-stack",
			);
			expect(fullStackOpportunity).toBeDefined();
			expect(fullStackOpportunity?.repositories).toEqual([
				"frontend",
				"backend",
			]);
			expect(fullStackOpportunity?.title).toContain("Full-Stack");
			expect(fullStackOpportunity?.benefits).toContain(
		...
		it("should identify microservices opportunities", async () => {
			...
			const microservicesOpportunity = opportunities.find(
				(opp) => opp.type === "microservices",
			);
		...
		it("should identify library ecosystem opportunities", async () => {
			...
			const libraryOpportunity = opportunities.find(
				(opp) => opp.type === "library-ecosystem",
			);
			expect(libraryOpportunity).toBeDefined();
			expect(libraryOpportunity?.title).toContain("Library Ecosystem");
			expect(libraryOpportunity?.benefits).toContain(
		...
		it("should identify mobile-backend opportunities", async () => {
			...
			const mobileOpportunity = opportunities.find(
				(opp) => opp.type === "mobile-backend",
			);
		...
		it("should sort opportunities by priority", async () => {
			...
			for (let i = 1; i < opportunities.length; i++) {
				expect(opportunities[i - 1].priority).toBeGreaterThanOrEqual(
					opportunities[i].priority,
				);
			}
		});
	});
	...
	describe("generateRelationshipInsights", () => {
		it("should generate comprehensive relationship insights", async () => {
			...
			expect(insights.integrationOpportunities.length).toBeGreaterThanOrEqual(
				0,
			);
		});
		...
		it("should calculate architectural patterns correctly", async () => {
			...
			const restPattern = insights.architecturalPatterns.find(
				(p) => p.pattern === "REST API",
			);
