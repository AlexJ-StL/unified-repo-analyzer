...
export class PathHandler {
   ...
   */
  public async validatePath(inputPath: string, options?: TimeoutOptions): Promise<PathValidationResult> {
   ...
   */
  public async checkPermissions(pathToCheck: string, options?: TimeoutOptions): Promise<PermissionResult> {
    const timeoutMs = options?.timeoutMs || this.defaultTimeoutMs;
    const signal = options?.signal;
   ...
   */
  private async checkPermissionsInternal(pathToCheck: string, signal?: AbortSignal): Promise<PermissionResult> {
   ...
   */
  private async checkBasicPermissions(pathToCheck: string, result: PermissionResult): Promise<void> {
   ...
   */
  private async checkWindowsPermissions(pathToCheck: string, stats: any, result: PermissionResult): Promise<void> {
    try {
    ...
    if (this.isWindows) {
      ...
      if (inputPath.length > this.windowsMaxPathLength) {
        errors.push({
          ...
          suggestions: ['Use shorter directory names', 'Move files closer to root directory', 'Enable long path support in Windows']
        });
      }
      ...
      if (invalidChars.test(inputPath)) {
        const foundChars = inputPath.match(/[<>:"|?*]/g);
        errors.push({
   ...
   */
  private async checkPathExists(pathToCheck: string): Promise<{ exists: boolean; isDirectory: boolean; size?: number }> {
    try {
