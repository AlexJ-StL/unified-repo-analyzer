...
export class ErrorMessageService {
	...
	public createPathErrorMessage(
		...
		switch (primaryError.code) {
			...
			case "WINDOWS_PERMISSION_CHECK_ERROR":
				return this.createWindowsPermissionCheckErrorMessage(
					path,
					primaryError,
				);
	 ...
	 */
	public createNetworkErrorMessage(
		path: string,
		errorDetails?: string,
	): ErrorMessageTemplate {
	 ...
	 */
	public createPermissionCheckErrorMessage(
		path: string,
		error: PathError,
	): ErrorMessageTemplate {
		return {
	 ...
	 */
	public createOwnershipInfoErrorMessage(
		path: string,
		error: PathError,
	): ErrorMessageTemplate {
		return {
	...
	private createPathNotFoundMessage(path: string): ErrorMessageTemplate {
		return {
			...
			details: `We searched for the path but could not find it on your system. This could be due to a typo, missing folder, or access restrictions.`,
			suggestions: this.getPlatformSpecificSuggestions({
				windows: [
	 ...
	 */
	private createInvalidFormatMessage(
		path: string,
		errors: PathError[],
	): ErrorMessageTemplate {
	 ...
	 */
	private createPathTooLongMessage(
		path: string,
		errors: PathError[],
	): ErrorMessageTemplate {
	 ...
	 */
	private createReservedNameMessage(
		path: string,
		errors: PathError[],
	): ErrorMessageTemplate {
	 ...
	 */
	private createInvalidCharactersMessage(
		path: string,
		errors: PathError[],
	): ErrorMessageTemplate {
	...
	private getPlatformSpecificSuggestions(guidance: PlatformGuidance): string[] {
		const platformSuggestions =
			guidance[this.currentPlatform as keyof PlatformGuidance] || [];
