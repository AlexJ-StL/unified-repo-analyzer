...
import { discoverRepository, analysisOptionsToDiscoveryOptions, } from '../utils/repositoryDiscovery';
...
export class AnalysisEngine {
    ...
    async analyzeRepository(repoPath, options) {
        ...
        return deduplicationService.deduplicateAnalysis(repoPath, options, async () => {
            ...
            if (cached) {
                timer();
                metricsService.recordAnalysis(repoPath, options.mode, 0, // No processing time for cached results
                cached.fileCount, cached.totalSize, true, // Cache hit
            ...
            timer();
            metricsService.recordAnalysis(repoPath, options.mode, processingTime, analysis.fileCount, analysis.totalSize, false, // Not a cache hit
            false // Not deduplicated (this is the original request)
            );
            return analysis;
        });
     ...
     */
    async analyzeMultipleRepositories(repoPaths, options) {
        const startTime = Date.now();
        ...
        for (let i = 0; i < repoPaths.length; i++) {
            ...
            batchResult.status.progress = Math.round(((batchResult.status.completed + batchResult.status.failed) / batchResult.status.total) *
                100);
            try {
                // Analyze repository
     ...
     * @returns Promise resolving to batch analysis result
     */
    async analyzeMultipleRepositoriesWithQueue(repoPaths, options, concurrency = 2, progressCallback) {
        ...
        return deduplicationService.deduplicateBatch(repoPaths, options, async () => {
            ...
            const queue = new TaskQueue(async (repoPath) => this.analyzeRepository(repoPath, options), { concurrency });
            // Create batch analysis result
            const batchResult = {
                id: batchId,
                repositories: [],
     ...
     */
    async generateCombinedInsights(repositories) {
        // Find common languages
        const languageSets = repositories.map((repo) => new Set(repo.languages));
        const commonLanguages = Array.from(languageSets.reduce((common, languages) => {
        ...
        const commonFrameworks = Array.from(frameworkSets.reduce((common, frameworks) => {
        ...
        if (commonLanguages.length > 0 || commonFrameworks.length > 0) {
            integrationOpportunities.push('Repositories share common technologies which could facilitate integration');
        }
        // Check for frontend/backend pairs
        const hasFrontend = repositories.some((repo) => repo.frameworks.some((f) => ['react', 'vue', 'angular', 'svelte'].includes(f.toLowerCase())));
        const hasBackend = repositories.some((repo) => repo.frameworks.some((f) => ['express', 'nest', 'django', 'flask', 'spring'].includes(f.toLowerCase())));
     ...
     */
    async generateSynopsis(analysis, format) {
     ...
     */
    async processFilesForAnalysis(analysis, options) {
        // Initialize counters
