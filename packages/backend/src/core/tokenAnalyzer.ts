...
export function sampleText(
  text: string,
  maxTokens: number,
  strategy: 'start' | 'end' | 'middle' | 'smart' = 'smart'
): string {
  if (!text) return '';
 ...
 * @returns Sampled text
 */
function smartSample(text: string, maxTokens: number): string {
  // Split text into sections
  const lines = text.split('\n');

  // Identify important sections
  const importantSections: { start: number; end: number; importance: number }[] = [];
  ...
  for (const index of lineIndices) {
    if (index === -1) continue; // Skip lines not found in original text
