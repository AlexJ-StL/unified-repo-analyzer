...
describe("AdvancedAnalyzer", () => {
  ...
  describe("analyzeRepository", () => {
    it("should perform comprehensive advanced analysis", async () => {
      ...
      expect(result.codeQuality.fileMetrics.length).toBe(
        mockAnalysis.structure.keyFiles.length
      );
      ...
      expect(result.architecture.maintainabilityScore).toBeGreaterThanOrEqual(
        0
      );
    ...
    it("should handle files with no content gracefully", async () => {
      ...
      expect(result.codeQuality.fileMetrics).toHaveLength(
        mockAnalysis.structure.keyFiles.length
      );
  ...
  describe("Security Analysis", () => {
    ...
    it("should detect SQL injection vulnerabilities", async () => {
      ...
      const sqlInjectionVulns = result.security.vulnerabilities.filter(
        (v) => v.type === "Potential SQL Injection"
      );
    ...
    it("should detect XSS vulnerabilities", async () => {
      ...
      const xssVulns = result.security.vulnerabilities.filter(
        (v) => v.type.includes("XSS") || v.type.includes("eval")
      );
  ...
  describe("Architectural Pattern Detection", () => {
    ...
    it("should generate architectural recommendations", async () => {
      ...
      expect(
        result.architecture.recommendations.some(
          (r) => r.includes("layered architecture") || r.includes("component")
        )
      ).toBe(true);
    });
  });
