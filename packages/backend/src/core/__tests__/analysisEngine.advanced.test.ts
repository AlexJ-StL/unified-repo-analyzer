...
vi.mock("../../services/deduplication.service", () => ({
  deduplicationService: {
    deduplicateAnalysis: vi
      .fn()
      .mockImplementation(
        async (repoPath: string, options: any, executor: () => Promise<any>) =>
          await executor()
      )
  }
}));
      ...
      security: {
        vulnerabilities: [
          ...
          },
          {
            ...
            type: "code-injection",
            description:
              "Use of eval() detected which can lead to code injection",
            file: "src/app.js",
            line: 10,
...
describe("AnalysisEngine Advanced Features Integration", () => {
  ...
  beforeEach(() => {
    ...
    mockAnalysisOptionsToDiscoveryOptions = vi.mocked(
      analysisOptionsToDiscoveryOptions
    );
  ...
  describe("Security Integration", () => {
    it("should add high-severity security issues to potential issues", async () => {
      ...
      const securityIssues = result.insights.potentialIssues.filter(
        (issue: string) => issue.startsWith("Security:")
      );

      expect(securityIssues.length).toBeGreaterThan(0);
      expect(
        securityIssues.some((issue: string) => issue.includes("SQL Injection"))
      ).toBe(true);
    });
    ...
    it("should add security recommendations", async () => {
      ...
      expect(
        recommendations.some((rec: string) => rec.includes("helmet.js"))
      ).toBe(true);
      expect(
        recommendations.some((rec: string) => rec.includes("rate limiting"))
      ).toBe(true);
      expect(recommendations.some((rec: string) => rec.includes("XSS"))).toBe(
        true
      );
    });
  });
  ...
  describe("Code Quality Integration", () => {
    it("should add high-severity quality issues to potential issues", async () => {
      ...
      const qualityIssues = result.insights.potentialIssues.filter(
        (issue: string) => issue.startsWith("Quality:")
      );

      expect(qualityIssues.length).toBeGreaterThan(0);
      expect(
        qualityIssues.some((issue: string) => issue.includes("too long"))
      ).toBe(true);
    });
  ...
  describe("Architectural Pattern Integration", () => {
    it("should detect and add architectural patterns", async () => {
      ...
      const patternNames = result.codeAnalysis.patterns.map(
