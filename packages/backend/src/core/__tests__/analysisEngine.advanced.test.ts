...
vi.mock('../../services/deduplication.service', () => ({
  deduplicationService: {
    deduplicateAnalysis: vi
      .fn()
      .mockImplementation(
        async (_repoPath: string, _options: any, executor: () => Promise<any>) => await executor()
      ),
  },
}));
...
vi.mock('../advancedAnalyzer', () => ({
  AdvancedAnalyzer: vi.fn().mockImplementation(() => ({
    analyzeRepository: vi.fn().mockResolvedValue({
        ...
        technicalDebt: [
          {
            type: 'complexity',
            severity: 'high',
            description: 'Method complexFunction is too long (20 lines)',
            file: 'src/app.js',
            estimatedEffort: 2,
      ...
      security: {
        ...
        securityScore: 45,
        recommendations: [
          'Use environment variables for sensitive configuration like passwords and API keys',
          ...
          'Consider using helmet.js for Express applications to set security headers',
        ],
      },
      architecture: {
        patterns: [
          {
            ...
            description:
              'The codebase follows MVC architectural pattern with separate models, views, and controllers',
            confidence: 0.8,
            locations: ['src/models/', 'src/views/', 'src/controllers/'],
          },
        ],
        recommendations: [
          'Consider implementing a layered architecture to separate concerns more clearly',
          'Add dependency injection to improve testability and maintainability',
          'Implement proper error boundaries in React components for better user experience',
        ],
...
describe('AnalysisEngine Advanced Features Integration', () => {
  ...
  let mockReadFile: MockedFunction<typeof readFileWithErrorHandling>;
  let mockDiscoverRepository: MockedFunction<typeof discoverRepository>;
  let mockAnalysisOptionsToDiscoveryOptions: MockedFunction<
    typeof analysisOptionsToDiscoveryOptions
  >;
  ...
  beforeEach(() => {
    ...
    mockReadFile = vi.mocked(readFileWithErrorHandling);
    mockDiscoverRepository = vi.mocked(discoverRepository);
    mockAnalysisOptionsToDiscoveryOptions = vi.mocked(analysisOptionsToDiscoveryOptions);
  ...
  describe('Security Integration', () => {
    it('should add high-severity security issues to potential issues', async () => {
      ...
      const securityIssues = result.insights.potentialIssues.filter((issue: string) =>
        issue.startsWith('Security:')
      );

      expect(securityIssues.length).toBeGreaterThan(0);
      expect(securityIssues.some((issue: string) => issue.includes('SQL Injection'))).toBe(true);
    });
    ...
    it('should add security recommendations', async () => {
      ...
      expect(recommendations.some((rec: string) => rec.includes('helmet.js'))).toBe(true);
      expect(recommendations.some((rec: string) => rec.includes('rate limiting'))).toBe(true);
      expect(recommendations.some((rec: string) => rec.includes('XSS'))).toBe(true);
    });
  });
  ...
  describe('Code Quality Integration', () => {
    it('should add high-severity quality issues to potential issues', async () => {
      ...
      const qualityIssues = result.insights.potentialIssues.filter((issue: string) =>
        issue.startsWith('Quality:')
      );

      expect(qualityIssues.length).toBeGreaterThan(0);
      expect(qualityIssues.some((issue: string) => issue.includes('too long'))).toBe(true);
    });
  ...
  describe('Architectural Pattern Integration', () => {
    it('should detect and add architectural patterns', async () => {
      ...
      const patternNames = result.codeAnalysis.patterns.map((p: { name: string }) => p.name);
    ...
    it('should add architectural recommendations', async () => {
      ...
      expect(
        recommendations.some(
          (rec: string) => rec.includes('layered architecture') || rec.includes('component')
        )
      ).toBe(true);
    });
  });
  ...
  describe('Performance and Error Handling', () => {
    ...
    it('should handle malformed JSON in package.json', async () => {
      (mockReadFile as any).mockImplementation((filePath: string) => {
