...
describe('AnalysisEngine Advanced Features (Fixed)', () => {
  ...
  beforeEach(() => {
    ...
    mockReadFileWithErrorHandling.mockResolvedValue('// Advanced test file content');
    ...
    mockDiscoverRepository.mockResolvedValue({
      ...
      insights: {
        executiveSummary: 'Modern React application with TypeScript and Express backend',
        technicalBreakdown: 'Well-structured codebase with good separation of concerns',
        recommendations: [
      ...
      metadata: {
        ...
        llmAnalysis: {
          ...
          recommendations: ['Refactor complex functions', 'Add comprehensive testing'],
          codeQuality: 'good',
    ...
    mockSampleText.mockImplementation((text: string) => `${text.substring(0, 100)}...`);
  ...
  describe('LLM Integration Features', () => {
    ...
    it('should provide AI-generated insights', async () => {
      ...
      expect(repo.insights.executiveSummary).toContain('Modern React application');
      expect(repo.insights.technicalBreakdown).toContain('Well-structured codebase');
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle file reading errors gracefully', async () => {
      mockReadFileWithErrorHandling.mockRejectedValueOnce(new Error('File not accessible'));

      await expect(mockReadFileWithErrorHandling('/protected/file.ts')).rejects.toThrow(
        'File not accessible'
      );
    });

    it('should handle repository discovery failures', async () => {
      mockDiscoverRepository.mockRejectedValueOnce(new Error('Repository analysis failed'));
