...
describe('AnalysisEngine - Batch Processing', () => {
    ...
    test('should generate combined insights for multiple repositories', async () => {
        ...
        expect(result.combinedInsights?.commonalities).toContain('All repositories use the following languages: JavaScript, TypeScript');
        expect(result.combinedInsights?.differences).toBeDefined();
        expect(result.combinedInsights?.integrationOpportunities).toContain('Potential for frontend-backend integration detected');
    });
    test('should analyze multiple repositories with queue', async () => {
        const repoPaths = ['/path/to/repo1', '/path/to/repo2', '/path/to/repo3'];
        // Mock progress callback
        const progressCallback = jest.fn();
        const result = await engine.analyzeMultipleRepositoriesWithQueue(repoPaths, options, 2, progressCallback);
        expect(result).toBeDefined();
        expect(result.repositories).toHaveLength(3);
        expect(progressCallback).toHaveBeenCalled();
    });
});
//# sourceMappingURL=AnalysisEngine.batch.test.js.map