...
describe('IndexSystem Relationship Functionality', () => {
    ...
    describe('calculateSimilarity', () => {
        it('should calculate high similarity for repositories with shared languages and frameworks', () => {
            ...
            const similarity = indexSystem.calculateSimilarity(repoA, repoB);
        ...
        it('should identify dependency relationships for library-application pairs', () => {
            ...
            const similarity = indexSystem.calculateSimilarity(libraryRepo, appRepo);
    ...
    describe('suggestCombinations', () => {
        beforeEach(async () => {
            ...
            const repositories = [
                ...
                createMockAnalysis('backend-1', 'Express Backend', ['javascript', 'typescript'], ['express']),
                createMockAnalysis('mobile-1', 'React Native App', ['javascript', 'typescript'], ['react-native']),
        ...
        it('should suggest full-stack combinations', async () => {
            ...
            const fullStackSuggestion = suggestions.find((s) => s.repositories.includes('frontend-1') && s.repositories.includes('backend-1'));
        ...
        it('should suggest microservices combinations', async () => {
            ...
            const suggestions = await indexSystem.suggestCombinations(repoIds);
            expect(suggestions.length).toBeGreaterThan(0);
            const microservicesSuggestion = suggestions.find((s) => s.repositories.includes('service-1') && s.repositories.includes('service-2'));
        ...
        it('should suggest library ecosystem combinations', async () => {
            ...
            const suggestions = await indexSystem.suggestCombinations(repoIds);
            expect(suggestions.length).toBeGreaterThan(0);
            const librarySuggestion = suggestions.find((s) => s.repositories.includes('library-1') && s.repositories.includes('frontend-1'));
        ...
        it('should sort suggestions by compatibility score', async () => {
            ...
            for (let i = 1; i < suggestions.length; i++) {
                expect(suggestions[i - 1].compatibility).toBeGreaterThanOrEqual(suggestions[i].compatibility);
            }
        });
        it('should limit suggestions to top 10', async () => {
            ...
            const moreRepos = [];
            for (let i = 0; i < 10; i++) {
                moreRepos.push(createMockAnalysis(`repo-${i}`, `Repository ${i}`, ['javascript'], ['express']));
            }
            for (const repo of moreRepos) {
                await indexSystem.addRepository(repo);
            }
            const allRepoIds = moreRepos.map((r) => r.id);
    ...
    describe('repository type detection', () => {
        it('should correctly identify frontend repositories', () => {
            ...
            const isFrontend = indexSystem.isFrontendRepo(frontendRepo);
        ...
        it('should correctly identify backend repositories', () => {
            ...
            const isBackend = indexSystem.isBackendRepo(backendRepo);
        ...
        it('should correctly identify mobile repositories', () => {
            ...
            const isMobile = indexSystem.isMobileRepo(mobileRepo);
        ...
        it('should correctly identify library repositories', () => {
            ...
            const isLibrary = indexSystem.isLibraryRepo(libraryRepo);
            expect(isLibrary).toBe(true);
        });
    });
});
// Helper function to create mock repository analysis
function createMockAnalysis(id, name, languages, frameworks) {
    return {
        id,
        path: `/path/to/${id}`,
        name,
...
//# sourceMappingURL=IndexSystem.relationship.test.js.map