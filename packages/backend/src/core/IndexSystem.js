...
export class IndexSystem {
        ...
        if (this.indexPath && fs.existsSync(this.indexPath)) {
            try {
                ...
                if (loadedIndex.repositories) {
                    loadedIndex.repositories.forEach((repo) => {
    ...
    async addRepository(analysis) {
        ...
        const existingIndex = this.index.repositories.findIndex((repo) => repo.path === indexedRepo.path);
        if (existingIndex >= 0) {
     ...
     */
    async updateRepository(repoId, analysis) {
        // Extract metadata from analysis
    ...
    async searchRepositories(query) {
        ...
        if (query.languages && query.languages.length > 0) {
            results = results.filter((result) => query.languages.some((lang) => result.repository.languages.includes(lang)));
            // Add language matches to results
            results.forEach((result) => {
                const matchedLanguages = query.languages.filter((lang) => result.repository.languages.includes(lang));
        ...
        if (query.frameworks && query.frameworks.length > 0) {
            results = results.filter((result) => query.frameworks.some((framework) => result.repository.frameworks.includes(framework)));
            // Add framework matches to results
            results.forEach((result) => {
                const matchedFrameworks = query.frameworks.filter((framework) => result.repository.frameworks.includes(framework));
        ...
        if (query.keywords && query.keywords.length > 0) {
            results = results.filter((result) => {
                ...
                return query.keywords.some((keyword) => result.repository.name.toLowerCase().includes(keyword.toLowerCase()) ||
            ...
            results.forEach((result) => {
                query.keywords.forEach((keyword) => {
                    ...
                    if (result.repository.summary.toLowerCase().includes(lowerKeyword)) {
                        result.matches.push({
                            field: 'summary',
                            value: `...${result.repository.summary.substring(Math.max(0, result.repository.summary.toLowerCase().indexOf(lowerKeyword) - 20), Math.min(result.repository.summary.length, result.repository.summary.toLowerCase().indexOf(lowerKeyword) +
                                keyword.length +
                                20))}...`,
                            score: 15,
                        });
                    ...
                    // Check tag match
                    const matchedTags = result.repository.tags.filter((tag) => tag.toLowerCase().includes(lowerKeyword));
        ...
        if (query.fileTypes && query.fileTypes.length > 0) {
            ...
            results = results.filter((result) => {
                return query.fileTypes.some((fileType) => {
                    ...
                    return result.repository.tags.some((tag) => tag.toLowerCase().includes(fileType.toLowerCase().replace('.', '')));
                });
            });
        ...
        if (query.dateRange) {
            results = results.filter((result) => result.repository.lastAnalyzed >= query.dateRange.start &&
     ...
     */
    async findSimilarRepositories(repoId) {
        // Find repository in index
        ...
        const relationships = this.index.relationships.filter((rel) => rel.sourceId === repoId || rel.targetId === repoId);
        // Convert relationships to matches
        const matches = relationships.map((rel) => {
     ...
     */
    async suggestCombinations(repoIds) {
     ...
     */
    async analyzeCombination(repositories) {
        ...
        const avgPairwiseCompatibility = pairwiseScores.length > 0
     ...
     */
    analyzeArchitecturalCompatibility(repositories) {
        const rationale = [];
        ...
        const serviceRepos = repositories.filter((repo) => repo.tags.some((tag) => tag.toLowerCase().includes('service')) ||
     ...
     */
    analyzeTechStackSynergy(repositories) {
        const rationale = [];
        const integrationPoints = [];
        let score = 0;
        ...
        const strongestEcosystem = Object.entries(ecosystemScores).reduce((max, [ecosystem, score]) => (score > max.score ? { ecosystem, score } : max), { ecosystem: '', score: 0 });
        ...
        for (const pair of complementaryPairs) {
            const hasFrontend = pair.frontend && allFrameworks.some((fw) => pair.frontend.includes(fw.toLowerCase()));
            const hasBackend = pair.backend && allFrameworks.some((fw) => pair.backend.includes(fw.toLowerCase()));
     ...
     */
    analyzeFunctionalityComplementarity(repositories) {
        const rationale = [];
        const integrationPoints = [];
        let score = 0;
     ...
     */
    analyzeWorkflowCompatibility(repositories) {
        const rationale = [];
        const integrationPoints = [];
        let score = 0;
        ...
        const complexityVariance = complexities.reduce((sum, c) => sum + Math.pow(c - avgComplexity, 2), 0) /
            complexities.length;
    ...
    inferRepositoryPurpose(repo) {
        ...
        if (repo.frameworks.some((fw) => ['express', 'nest', 'django', 'flask'].includes(fw.toLowerCase()))) {
            purposes.push('backend');
        }
        if (repo.frameworks.some((fw) => ['react-native', 'flutter', 'ionic'].includes(fw.toLowerCase()))) {
     ...
     */
    extractRepositoryMetadata(analysis) {
        // Calculate complexity score (simplified)
        const complexity = analysis.codeAnalysis.complexity?.maintainabilityIndex ||
     ...
     */
    async detectRelationships(repository) {
        // Remove existing relationships for this repository
        this.index.relationships = this.index.relationships.filter((rel) => rel.sourceId !== repository.id && rel.targetId !== repository.id);
        // Compare with all other repositories
        for (const otherRepo of this.index.repositories) {
            // Skip self-comparison
            if (otherRepo.id === repository.id)
                continue;
     ...
     */
    async updateRelationships(repository) {
     ...
     */
    calculateSimilarity(repoA, repoB) {
        let score = 0;
        let reasons = [];
     ...
     */
    calculateLanguageSimilarity(repoA, repoB) {
        const reasons = [];
        let score = 0;
        ...
        if (sharedLanguages.length > 0) {
            ...
            const maxPossibleWeight = Math.max(repoA.languages.reduce((sum, lang) => sum + (languageWeights[lang.toLowerCase()] || 0.3), 0), repoB.languages.reduce((sum, lang) => sum + (languageWeights[lang.toLowerCase()] || 0.3), 0));
        ...
        for (const [langA, langB] of complementaryPairs) {
            if ((repoA.languages.some((l) => l.toLowerCase() === langA) &&
     ...
     */
    calculateFrameworkCompatibility(repoA, repoB) {
        const reasons = [];
        let score = 0;
        ...
        if (sharedFrameworks.length > 0) {
            const frameworkScore = sharedFrameworks.length / Math.max(repoA.frameworks.length, repoB.frameworks.length);
     ...
     */
    calculateArchitecturalSimilarity(repoA, repoB) {
        const reasons = [];
        let score = 0;
        // Extract architectural patterns from tags and frameworks
        const getArchitecturalPatterns = (repo) => {
            const patterns = [];
            ...
            if (repo.frameworks.some((f) => ['express', 'nest', 'django', 'flask'].includes(f.toLowerCase()))) {
            ...
            if (repo.tags.some((tag) => microserviceIndicators.some((indicator) => tag.toLowerCase().includes(indicator)))) {
     ...
     */
    calculateTechStackCompatibility(repoA, repoB) {
        const reasons = [];
        let score = 0;
        ...
        for (const [ecosystem, technologies] of Object.entries(ecosystems)) {
            const repoAInEcosystem = repoA.languages.some((lang) => lang.toLowerCase().includes(ecosystem)) ||
                repoA.frameworks.some((fw) => technologies.some((tech) => fw.toLowerCase().includes(tech)));
            const repoBInEcosystem = repoB.languages.some((lang) => lang.toLowerCase().includes(ecosystem)) ||
     ...
     */
    calculateStructureSimilarity(repoA, repoB) {
        const reasons = [];
        let score = 0;
        // Size similarity
        ...
        const complexityRatio = Math.min(repoA.complexity, repoB.complexity) / Math.max(repoA.complexity, repoB.complexity);
     ...
     */
    calculateSemanticSimilarity(repoA, repoB) {
        const reasons = [];
        let score = 0;
        ...
        if (commonKeywords.length > 0) {
            const keywordSimilarity = commonKeywords.length / Math.max(keywordsA.length, keywordsB.length);
     ...
     */
    determineRelationshipType(repoA, repoB, score, _reasons) {
     ...
     */
    areComplementaryRepositories(repoA, repoB) {
        // Frontend-Backend complementarity
        if ((this.isFrontendRepo(repoA) && this.isBackendRepo(repoB)) ||
            (this.isBackendRepo(repoA) && this.isFrontendRepo(repoB))) {
            return true;
        }
        // Library-Application complementarity
        if ((this.isLibraryRepo(repoA) && this.isApplicationRepo(repoB)) ||
            (this.isApplicationRepo(repoA) && this.isLibraryRepo(repoB))) {
            return true;
        }
        // Mobile-Backend complementarity
        if ((this.isMobileRepo(repoA) && this.isBackendRepo(repoB)) ||
     ...
     */
    isDependencyRelationship(repoA, repoB) {
    ...
    isFrontendRepo(repo) {
        ...
        return repo.frameworks.some((fw) => frontendFrameworks.some((frontFw) => fw.toLowerCase().includes(frontFw)));
    }
    /**
    ...
    isBackendRepo(repo) {
        ...
        return repo.frameworks.some((fw) => backendFrameworks.some((backFw) => fw.toLowerCase().includes(backFw)));
    }
    /**
    ...
    isLibraryRepo(repo) {
        ...
        const libraryTags = repo.tags.filter((tag) => ['library', 'package', 'module', 'component', 'utility'].some((indicator) => tag.toLowerCase().includes(indicator)));
        ...
        const hasLibraryFrameworks = repo.frameworks.some((fw) => ['npm', 'yarn', 'webpack', 'rollup', 'babel'].includes(fw.toLowerCase()));
    ...
    isMobileRepo(repo) {
        ...
        if (repo.frameworks.some((fw) => mobileFrameworks.some((mobileFw) => fw.toLowerCase().includes(mobileFw)))) {
            return true;
        }
        return false;
    }
    ...
    isToolRepo(repo) {
        ...
        const toolTags = repo.tags.filter((tag) => ['tool', 'cli', 'automation', 'build', 'deployment'].some((indicator) => tag.toLowerCase().includes(indicator)));
     ...
     */
    async addRepositoryTag(repoId, tagName, category, color) {
        // Find repository in index
     ...
     */
    async removeRepositoryTag(repoId, tagName) {
        // Find repository in index
        const repoIndex = this.index.repositories.findIndex((repo) => repo.id === repoId);
        if (repoIndex < 0) {
            throw new Error(`Repository with ID ${repoId} not found in index`);
        ...
        this.index.repositories[repoIndex].tags = this.index.repositories[repoIndex].tags.filter((tag) => tag !== tagName);
        // Update index timestamp
        this.index.lastUpdated = new Date();
        // Save index to disk
        await this.saveIndex();
     ...
     */
    async addTag(name, category, color) {
...
//# sourceMappingURL=IndexSystem.js.map