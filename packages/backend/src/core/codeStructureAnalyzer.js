...
const STRUCTURE_PATTERNS = {
    ...
    javascript: {
        ...
        function: /(?:function\s+(\w+)|(?:const|let|var)\s+(\w+)\s*=\s*function|(?:async\s+)?(?:function)?\s*(\w+)\s*\([^)]*\)\s*=>|(\w+)\s*:\s*function)/g,
    ...
    java: {
        ...
        function: /(?:public|private|protected|static|\s) +[\w<>[\]]+\s+(\w+) *\([^)]*\) *(?:{|throws)/g,
    ...
    csharp: {
        // Method declarations: public void Name(...) or private string Name(...)
        function: /(?:public|private|protected|internal|static|\s) +[\w<>[\]]+\s+(\w+) *\([^)]*\) *(?:{|=>)/g,
        // Class declarations: public class Name or class Name
