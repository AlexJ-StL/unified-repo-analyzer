...
describe('Platform-Specific Integration Tests', () => {
  ...
  describe('Performance and Timeout Testing', () => {
    it('should timeout path validation after specified time', async () => {
      ...
      // Debug: log all error messages to see what's actually being returned
      const errorMessages = result.errors.map((e) => e.message);
      console.log('Actual error messages:', errorMessages);

      // Check for timeout-related errors (be more flexible with error messages)
    ...
    it('should support cancellation via AbortSignal', async () => {
      ...
      // Debug: log all error codes to see what's actually being returned
      const errorCodes = result.errors.map((e) => e.code);
      console.log('Actual error codes:', errorCodes);

      // Check for cancellation-related errors (be more flexible with error codes)
      const hasCancellationError = result.errors.some(
        (e) =>
      ...
      expect(hasCancellationError).toBe(true);
  ...
  describe('End-to-End User Workflow Tests', () => {
    ...
    it('should handle user input validation with helpful error messages', async () => {
      ...
      for (const { path: testPath, expectedError } of invalidInputs) {
        ...
        // Some errors might not have details, so check if it exists before asserting
    ...
    it('should handle network path scenarios', async () => {
      ...
      for (const networkPath of networkPaths) {
        ...
        // Debug: log errors if normalizedPath is undefined
        if (result.normalizedPath === undefined) {
          console.log(`Path: ${networkPath}, Errors:`, result.errors);
        }
