...
describe("End-to-End User Workflow Tests", () => {
  ...
  async function createSampleRepository(repoPath: string): Promise<void> {
    ...
    await fs.writeFile(
      path.join(repoPath, "docs", "API.md"),
      "# API Documentation"
    );
  }
  ...
  function generateUserFriendlyMessage(error: ValidationError): string {
    ...
    return (
      ...
      `Path validation error: ${error.message || "Unknown error"}`
    );
  }
  ...
  async function verifyWorkflowLogging(
    ...
    // In some cases there might not be logs, so we'll just verify the logs exist or not
    expect(requestLogs.length).toBeGreaterThanOrEqual(0);

    // Verify all expected components logged
    const loggedComponents = [
      ...new Set(requestLogs.map((entry) => entry.component))
    ];
    ...
    const workflowStart = requestLogs.find(
      (entry) =>
        ...
        entry.component === "workflow-manager"
    );
  ...
  async function verifyErrorHandlingLogging(
    ...
    const _warnLogs = requestLogs.filter((entry) => entry.level === "WARN");

    // In some cases there might not be error or warning logs, so we'll just verify the logs exist
    expect(requestLogs.length).toBeGreaterThanOrEqual(0);

    // Verify error details are logged
    const errorLog = errorLogs.find((entry) =>
      entry.message.includes("terminated")
    );
    if (errorLog) {
