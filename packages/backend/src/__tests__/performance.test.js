...
describe('Performance Tests', () => {
    ...
    describe('Cache Performance', () => {
        ...
        it('should handle cache operations efficiently', async () => {
            ...
            console.log(`Cache set operations: ${((cacheOperations / setDuration) * 1000).toFixed(0)} ops/sec`);
            console.log(`Cache get operations: ${((cacheOperations / getDuration) * 1000).toFixed(0)} ops/sec`);
        ...
        it('should handle cache invalidation correctly', async () => {
            ...
            analysisCache.clear();
            // Verify cache miss
            const cachedAfterInvalidation = await analysisCache.getCachedAnalysis(testRepoPath, defaultOptions);
    ...
    describe('Deduplication Performance', () => {
        ...
        it('should handle different requests separately', async () => {
            const options1 = { ...defaultOptions, mode: 'quick' };
            const options2 = { ...defaultOptions, mode: 'comprehensive' };
    ...
    describe('Batch Processing Performance', () => {
        it('should process multiple repositories efficiently', async () => {
            ...
            try {
                const startTime = performance.now();
                const batchResult = await analysisEngine.analyzeMultipleRepositoriesWithQueue(repoPaths, defaultOptions, 2 // concurrency
                );
                const totalTime = performance.now() - startTime;
    ...
    describe('Metrics Collection Performance', () => {
        it('should collect metrics without significant overhead', async () => {
            ...
            const overhead = ((durationWithMetrics - durationWithoutMetrics) / durationWithoutMetrics) * 100;
    ...
    describe('Large Repository Performance', () => {
        it('should handle large repositories efficiently', async () => {
            ...
            try {
                ...
                console.log(`Performance: ${(result.fileCount / (duration / 1000)).toFixed(2)} files/second`);
            }
            finally {
                try {
...
async function createTestRepository(repoPath) {
    ...
    await writeFile(path.join(srcPath, 'index.js'), `
...
app.listen(3000, () => {
  ...
  `);
    await writeFile(path.join(srcPath, 'utils.js'), `
...
module.exports = { formatDate, validateEmail };
  `);
    ...
    await writeFile(path.join(srcPath, 'types.ts'), `
...
export interface Repository {
  ...
  `);
    // Create README
    await writeFile(path.join(repoPath, 'README.md'), `
# Test Repository
 ...
 */
async function createLargeTestRepository(repoPath) {
    await mkdir(repoPath, { recursive: true });
    // Create package.json
    await writeFile(path.join(repoPath, 'package.json'), JSON.stringify({
