...
describe('MockProvider', () => {
    describe('constructor', () => {
        ...
        test('should initialize with custom options', () => {
            ...
            expect(provider.mockResponse).toBe('Custom response');
            expect(provider.mockTokenUsage).toEqual({
                prompt: 200,
                completion: 100,
                total: 300,
            });
            expect(provider.delay).toBe(100);
            expect(provider.shouldFail).toBe(true);
            expect(provider.errorMessage).toBe('Custom error');
        });
    });
    describe('name', () => {
