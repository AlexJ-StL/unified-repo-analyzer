...
describe('GeminiProvider', () => {
    ...
    describe('constructor', () => {
        ...
        test('should initialize with default model if not provided', () => {
            ...
            expect(provider.config.model).toBe('gemini-pro');
        });
        test('should use provided model if specified', () => {
            ...
            expect(provider.config.model).toBe('gemini-ultra');
        });
    });
    describe('name', () => {
    ...
    describe('analyze', () => {
        test('should call Gemini API and return formatted response', async () => {
            ...
            expect(mockedAxios.post).toHaveBeenCalledWith('https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=test-key', {
                contents: [
                    {
                        parts: [
                            {
        ...
        test('should handle content filtering', async () => {
            ...
            mockedAxios.post.mockResolvedValueOnce(mockResponse);
            const provider = new GeminiProvider({ apiKey: 'test-key' });
            await expect(provider.analyze('Test prompt')).rejects.toThrow('Gemini content filtered: SAFETY');
        });
        test('should handle empty candidates', async () => {
            ...
            mockedAxios.post.mockResolvedValueOnce(mockResponse);
            const provider = new GeminiProvider({ apiKey: 'test-key' });
            await expect(provider.analyze('Test prompt')).rejects.toThrow('Gemini returned no candidates');
        });
        test('should handle API errors', async () => {
            ...
            const provider = new GeminiProvider({ apiKey: 'test-key' });
            await expect(provider.analyze('Test prompt')).rejects.toThrow('Gemini API error: 400 - {"error":"Invalid request"}');
        });
        test('should handle non-Axios errors', async () => {
            ...
            const provider = new GeminiProvider({ apiKey: 'test-key' });
            await expect(provider.analyze('Test prompt')).rejects.toThrow('Gemini API error: Network error');
        });
    });
});
