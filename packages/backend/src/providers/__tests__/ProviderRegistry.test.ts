...
describe("ProviderRegistry", () => {
  ...
  describe("built-in providers", () => {
    ...
    test("should create built-in provider instances", () => {
      ...
      const claudeProvider = registry.createProvider("claude", {
        apiKey: "test-key",
      });
      const geminiProvider = registry.createProvider("gemini", {
        apiKey: "test-key",
      });
  ...
  describe("registerProvider", () => {
    test("should register custom provider", () => {
      ...
      registry.registerProvider(
        "custom",
        (config) => new CustomProvider(config)
      );
  ...
  describe("fetchProviderModels", () => {
    test("should throw error for provider that does not support model fetching", async () => {
      const registry = ProviderRegistry.getInstance();

      await expect(
        registry.fetchProviderModels("mock", "test-key")
      ).rejects.toThrow("Provider 'mock' does not support model fetching");
    });
  });
});
