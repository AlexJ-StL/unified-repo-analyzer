...
describe('ClaudeProvider', () => {
    ...
    describe('constructor', () => {
        ...
        test('should initialize with default model if not provided', () => {
            ...
            expect(provider.config.model).toBe('claude-2.1');
        });
        test('should use provided model if specified', () => {
            ...
            expect(provider.config.model).toBe('claude-3-opus');
        });
    });
    describe('name', () => {
    ...
    describe('analyze', () => {
        test('should call Claude API and return formatted response', async () => {
            ...
            expect(mockedAxios.post).toHaveBeenCalledWith('https://api.anthropic.com/v1/complete', {
                prompt: 'Test prompt',
                model: 'claude-2.1',
        ...
        test('should handle API errors', async () => {
            ...
            const provider = new ClaudeProvider({ apiKey: 'test-key' });
            await expect(provider.analyze('Test prompt')).rejects.toThrow('Claude API error: 400 - {"error":"Invalid request"}');
        });
        test('should handle non-Axios errors', async () => {
            ...
            const provider = new ClaudeProvider({ apiKey: 'test-key' });
            await expect(provider.analyze('Test prompt')).rejects.toThrow('Claude API error: Network error');
        });
    });
});
