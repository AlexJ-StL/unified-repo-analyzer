...
const parseEnvironment = () => {
  try {
    ...
    if (error instanceof z.ZodError) {
      const _warningMessages = error.errors.map((err) => `${err.path.join('.')}: ${err.message}`);
...
export const getSecret = (key: keyof Environment): string | undefined => {
  return env[key] as string | undefined;
};

export const requireSecret = (key: keyof Environment): string => {
...
export const validateProductionConfig = (): void => {
  ...
  const requiredSecrets: (keyof Environment)[] = ['JWT_SECRET', 'SESSION_SECRET', 'ENCRYPTION_KEY'];
