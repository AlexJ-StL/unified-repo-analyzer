...
class PerformanceBenchmark {
     ...
     */
    async runBenchmark(name, operation, iterations = 1) {
    ...
    async benchmarkCache() {
        ...
        await this.runBenchmark('Cache Set Operations', () => {
            const key = `test-key-${Math.random()}`;
            const value = { data: 'test-data'.repeat(100) };
        ...
        for (let i = 0; i < 1000; i++) {
            ...
            testKeys.push(key);
            analysisCache.set(key, { data: `test-data-${i}` });
        }
        // Cache get operations
        let keyIndex = 0;
        await this.runBenchmark('Cache Get Operations', () => {
            const key = testKeys[keyIndex % testKeys.length];
            analysisCache.get(key);
            keyIndex++;
        }, 50000);
        // Cache invalidation patterns
        await this.runBenchmark('Cache Pattern Invalidation', () => {
    ...
    async benchmarkMetrics() {
        ...
        // Metric recording
        await this.runBenchmark('Metrics Recording', () => {
            metricsService.recordMetric(`test.metric.${Math.floor(Math.random() * 100)}`, Math.random() * 1000, { tag: 'benchmark' });
        }, 50000);
        // Request metrics recording
        await this.runBenchmark('Request Metrics Recording', () => {
            metricsService.recordRequest(`/api/test/${Math.floor(Math.random() * 10)}`, 'GET', 200, Math.random() * 100);
        }, 10000);
        // Analysis metrics recording
        await this.runBenchmark('Analysis Metrics Recording', () => {
            metricsService.recordAnalysis(`/test/repo/${Math.floor(Math.random() * 10)}`, 'standard', Math.random() * 1000, Math.floor(Math.random() * 100), Math.floor(Math.random() * 1000000), Math.random() > 0.5, Math.random() > 0.8);
        }, 5000);
    }
    /**
    ...
    async benchmarkDeduplication() {
        ...
        await this.runBenchmark('Deduplication Key Generation', () => {
        ...
        const promises = [];
        await this.runBenchmark('Concurrent Request Deduplication', async () => {
            // Simulate concurrent identical requests
            for (let i = 0; i < concurrentRequests; i++) {
                const promise = deduplicationService.deduplicateAnalysis('/test/repo/same', testOptions, async () => {
    ...
    async benchmarkAnalysis() {
        ...
        try {
            ...
            analysisCache.clear();
            await this.runBenchmark('Repository Analysis (Cold Cache)', async () => {
                await this.analysisEngine.analyzeRepository(testRepoPath, defaultOptions);
            }, 1);
            // Single repository analysis (warm cache)
            await this.runBenchmark('Repository Analysis (Warm Cache)', async () => {
                await this.analysisEngine.analyzeRepository(testRepoPath, defaultOptions);
            }, 10);
            // Batch analysis
            const batchPaths = [testRepoPath, testRepoPath, testRepoPath];
            await this.runBenchmark('Batch Repository Analysis', async () => {
                await this.analysisEngine.analyzeMultipleRepositoriesWithQueue(batchPaths, defaultOptions, 2);
            }, 1);
        }
        finally {
     ...
     */
    async createTestRepository(repoPath) {
...
module.exports = { Class${i} };
      `;
        ...
        // Create README
        await writeFile(path.join(repoPath, 'README.md'), `# Benchmark Test Repository
...
This is a test repository created for performance benchmarking.
    ...
    generateReport() {
        ...
        for (const result of this.results) {
            ...
            report.push(`| ${result.name} | ${result.duration.toFixed(2)} | ${result.opsPerSecond.toFixed(0)} | ${memoryDeltaMB} |`);
        }
        report.push('');
        // Detailed results
        ...
        for (const result of this.results) {
            ...
            report.push(`- **Memory usage before**: ${(result.memoryUsage.before.heapUsed / 1024 / 1024).toFixed(2)}MB`);
            report.push(`- **Memory usage after**: ${(result.memoryUsage.after.heapUsed / 1024 / 1024).toFixed(2)}MB`);
