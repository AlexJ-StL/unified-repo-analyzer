...
import { traverseDirectory, getCombinedIgnorePatterns, extractDirectoryInfo, FileSystemError, FileSystemErrorType, } from './fileSystem';
 ...
 */
export async function discoverRepository(repoPath, options = {}) {
    const { maxFiles = 1000, maxLinesPerFile = 1000, ignorePatterns = [], includeContent = false, includeTree = true, } = options;
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
            throw new FileSystemError(`Repository not found: ${normalizedPath}`, FileSystemErrorType.NOT_FOUND, normalizedPath);
        }
        else if (error.code === 'EACCES') {
            throw new FileSystemError(`Permission denied: ${normalizedPath}`, FileSystemErrorType.PERMISSION_DENIED, normalizedPath);
        }
        else {
            throw new FileSystemError(`Error accessing repository: ${normalizedPath}`, FileSystemErrorType.UNKNOWN, normalizedPath);
        }
    }
    ...
    const fileInfos = fileInfoResults.filter(Boolean);
 ...
 */
export function generateFileTree(basePath, files) {
    const tree = new Map();
    // Add root directory
    tree.set('/', []);
    // Process all files
 ...
 */
export function analysisOptionsToDiscoveryOptions(options) {
...
//# sourceMappingURL=repositoryDiscovery.js.map