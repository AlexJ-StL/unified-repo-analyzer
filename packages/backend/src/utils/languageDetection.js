...
export const LANGUAGES = [
    {
        ...
        filenames: ['package.json', '.eslintrc.js', '.eslintrc.json', '.babelrc', 'webpack.config.js'],
        frameworks: [
            {
                name: 'React',
                files: ['jsx', 'tsx', 'react'],
    ...
    },
    {
        ...
        filenames: ['tsconfig.json', '.eslintrc.ts'],
        frameworks: [
            {
                name: 'React',
                files: ['tsx', 'react'],
    ...
    },
    {
        ...
        frameworks: [
            {
                name: 'Spring',
                files: ['Application.java', 'Controller.java', 'Service.java', 'Repository.java'],
 ...
 */
export function detectLanguageFromShebang(content) {
 ...
 */
export async function detectLanguage(filePath, content) {
 ...
 */
export async function detectFrameworks(files, packageJsonPath) {
    ...
    for (const lang of LANGUAGES) {
        if (lang.frameworks) {
            for (const framework of lang.frameworks) {
                ...
                if (matches > 0) {
                    frameworks.set(framework.name, (frameworks.get(framework.name) || 0) + matches * 0.5);
                }
            }
        }
    }
                    ...
                    for (const framework of lang.frameworks) {
                        if (framework.dependencies) {
                            for (const dep of framework.dependencies) {
                                if (dependencies[dep]) {
                                    frameworks.set(framework.name, (frameworks.get(framework.name) || 0) + 1);
                                }
                            }
                        }
                    }
