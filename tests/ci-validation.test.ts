...
import { createRuntimeTestHelper } from "./runtime-test-helpers";
...
describe("CI/CD Configuration Validation", () => {
  ...
  describe("Runtime Test Helpers", () => {
    it("should provide runtime-specific test configuration", () => {
      const helper = createRuntimeTestHelper();
      const config = {
        runtime: EnvironmentDetector.isBun() ? "bun" : "node",
        platform: EnvironmentDetector.getPlatform(),
        fastTimeout: helper.getTimeout("fast"),
        normalTimeout: helper.getTimeout("normal"),
        slowTimeout: helper.getTimeout("slow")
      };
    ...
    it("should create runtime-aware timeouts", () => {
      const helper = createRuntimeTestHelper();
      const fastTimeout = helper.getTimeout("fast");
      const normalTimeout = helper.getTimeout("normal");
      const slowTimeout = helper.getTimeout("slow");
