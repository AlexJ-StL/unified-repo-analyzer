...
async function compileTypeScriptFile(filePath: string): Promise<{
	success: boolean;
	errors: string[];
	warnings: string[];
}> {
...
describe("Configuration Compilation Tests", () => {
	...
	describe("Type Definition Files", () => {
		...
		test("Config types should have proper interface structure", async () => {
			...
			expect(typesContent).toMatch(
				/PostCSSConfig\s*{[\s\S]*plugins:\s*Record<string,\s*unknown>/,
			);

			// Tailwind config should have required properties
			expect(typesContent).toMatch(
				/TailwindConfig\s*{[\s\S]*content:\s*string\[\]/,
			);
			expect(typesContent).toMatch(/TailwindConfig\s*{[\s\S]*theme:\s*{/);
			expect(typesContent).toMatch(
				/TailwindConfig\s*{[\s\S]*plugins:\s*any\[\]/,
			);

			// Test result should have status property
			expect(typesContent).toMatch(
				/TestResult\s*{[\s\S]*status:\s*'passed'\s*\|\s*'failed'/,
			);
		});
	});
	...
	describe("Configuration File Imports", () => {
		...
		test("Test runner should be importable", async () => {
			try {
				const testRunner = await import(
					join(projectRoot, "scripts/test-runner.ts")
				);
	...
	describe("Configuration Validation", () => {
		...
		test("Tailwind config should have valid content paths", async () => {
			...
			expect(
				config.default.content.some((path: string) =>
					path.includes("src/**/*"),
				),
			).toBe(true);
		});
