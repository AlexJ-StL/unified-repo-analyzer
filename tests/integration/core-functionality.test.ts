...
describe("Core Functionality Integration Tests", () => {
  ...
  describe("Analysis Workflow Integration", () => {
    test("should integrate analysis components", async () => {
      ...
      const mockFileSystem = createMock({
        validateRepositoryPath: mockFunction().mockReturnValue(
          "/validated/repo/path"
        ),
      ...
      const validatedPath =
        mockFileSystem.validateRepositoryPath(repositoryPath);
      ...
      const analysisResult =
        await mockAnalysisEngine.analyzeRepository(validatedPath);
      ...
      expect(mockFileSystem.validateRepositoryPath).toHaveBeenCalledWith(
        repositoryPath
      );
      ...
      expect(mockValidator.validateRepositoryAnalysis).toHaveBeenCalledWith(
        analysisResult
      );
    });
    ...
    test("should handle analysis errors gracefully", async () => {
      ...
      const mockAnalysisEngine = createMock({
        analyzeRepository: mockFunction().mockRejectedValue(
  ...
  describe("Search and Index Integration", () => {
    test("should integrate search and indexing components", async () => {
      ...
      const searchResults =
        await mockIndexSystem.searchRepositories(searchQuery);
      ...
      expect(mockSearchValidator.validateSearchQuery).toHaveBeenCalledWith(
        searchQuery
      );
      expect(mockIndexSystem.searchRepositories).toHaveBeenCalledWith(
        searchQuery
      );
  ...
  describe("Export and Format Integration", () => {
    test("should integrate export and formatting components", async () => {
      ...
      const mockExportSystem = createMock({
        exportToJSON: mockFunction().mockReturnValue(
          JSON.stringify(mockAnalysisData, null, 2)
        ),
      ...
      const markdownExport =
        mockExportSystem.exportToMarkdown(mockAnalysisData);
      ...
      // Test file writing
      const outputPath = await mockFileWriter.writeFile(
      ...
      // Verify all mocks were called
      expect(mockExportSystem.exportToJSON).toHaveBeenCalledWith(
        mockAnalysisData
      );
      expect(mockExportSystem.exportToMarkdown).toHaveBeenCalledWith(
        mockAnalysisData
      );
      expect(mockExportSystem.exportToHTML).toHaveBeenCalledWith(
        mockAnalysisData
      );
  ...
  describe("Configuration Integration", () => {
    test("should integrate configuration management", () => {
      ...
      expect(mockConfigSystem.updateConfiguration).toHaveBeenCalledWith(
        newConfig
      );
      expect(mockConfigSystem.validateConfiguration).toHaveBeenCalledWith(
        newConfig
      );
    });
  });
  ...
  describe("Error Handling Integration", () => {
    test("should integrate error classification and formatting", () => {
      ...
      const consoleFormat =
        mockErrorFormatter.formatForConsole(classifiedError);
      ...
      expect(mockErrorFormatter.formatForAPI).toHaveBeenCalledWith(
        classifiedError
      );
      expect(mockErrorFormatter.formatForConsole).toHaveBeenCalledWith(
        classifiedError
      );
    });
  });
