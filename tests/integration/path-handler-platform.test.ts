...
} from "../../packages/backend/src/services/path-handler.service.js";
	...
	describe("Windows Path Format Handling", () => {
		...
		it("should handle UNC paths correctly", async () => {
			...
			for (const testPath of testPaths) {
				...
				const uncErrors = result.errors.filter(
					(e) => e.code === "INVALID_UNC_PATH",
				);
		...
		it("should handle Windows path length limits", async () => {
			...
			const longPath =
				"C:\\" + "VeryLongDirectoryName".repeat(15) + "\\file.txt";
		...
		it("should detect invalid drive letters", async () => {
			...
			for (const invalidDrive of invalidDrives) {
				...
				const driveError = result.errors.find(
					(e) => e.code === "INVALID_DRIVE_LETTER",
				);
				expect(driveError?.suggestions).toBeDefined();
				expect(driveError?.suggestions?.some((s) => s.includes("C:\\"))).toBe(
					true,
				);
			}
		});
	...
	describe("Cross-Platform Compatibility", () => {
		...
		it("should handle macOS paths correctly", async () => {
			...
			for (const testPath of testPaths) {
				...
				const windowsErrors = result.errors.filter(
					(e) =>
						e.code === "RESERVED_NAME" || e.code === "INVALID_DRIVE_LETTER",
				);
				expect(windowsErrors).toHaveLength(0);
			}
		});
	...
	describe("Real File System Integration", () => {
		...
		it("should handle permission errors gracefully", async () => {
			const nonExistentPath = path.join(
				testDir,
				"non-existent-permission-test.txt",
			);

			const permissionResult =
				await pathHandler.checkPermissions(nonExistentPath);
			...
			expect(
				permissionResult.errors.some((e) => e.code === "PATH_NOT_FOUND"),
			).toBe(true);
		});
		...
		it("should resolve relative paths correctly", async () => {
			...
			const resolvedPath = pathHandler.resolveRelativePath(
				relativePath,
				basePath,
			);
	...
	describe("Error Handling and Recovery", () => {
		it("should provide helpful error messages for common issues", async () => {
			...
			for (const scenario of errorScenarios) {
				...
				expect(result.isValid).toBe(false);
				expect(
					result.errors.some((e) => e.code === scenario.expectedError),
				).toBe(true);

				const error = result.errors.find(
					(e) => e.code === scenario.expectedError,
				);
		...
		it("should handle null byte security issues", async () => {
			...
			expect(result.isValid).toBe(false);
			expect(result.errors.some((e) => e.code === "NULL_BYTE_IN_PATH")).toBe(
				true,
			);

			const securityError = result.errors.find(
				(e) => e.code === "NULL_BYTE_IN_PATH",
			);
		...
		it("should handle control characters in paths", async () => {
			...
			expect(result.isValid).toBe(false);
			expect(result.errors.some((e) => e.code === "CONTROL_CHARACTERS")).toBe(
				true,
			);

			const controlError = result.errors.find(
				(e) => e.code === "CONTROL_CHARACTERS",
			);
		...
		it("should handle invalid UNC paths", async () => {
			...
			for (const uncPath of invalidUNCPaths) {
				...
				const uncError = result.errors.find(
					(e) => e.code === "INVALID_UNC_PATH",
				);
		...
		it("should handle trailing spaces and dots on Windows", async () => {
			...
			for (const invalidPath of invalidPaths) {
				...
				const endingError = result.errors.find(
					(e) => e.code === "INVALID_COMPONENT_ENDING",
				);
	...
	describe("Edge Cases and Boundary Conditions", () => {
		...
		it("should handle paths with Unicode characters", async () => {
			...
			for (const unicodePath of unicodePaths) {
				...
				const formatErrors = result.errors.filter(
					(e) =>
						e.code === "INVALID_CHARACTERS" || e.code === "CONTROL_CHARACTERS",
				);
				expect(formatErrors).toHaveLength(0);
			}
		});
