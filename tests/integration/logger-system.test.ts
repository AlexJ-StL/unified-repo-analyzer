...
import { type LogEntry, Logger } from '../../packages/backend/src/services/logger.service.js';
...
describe('Logging System Integration Tests', () => {
  ...
  describe('Log Correlation Across Components', () => {
    it('should maintain request ID correlation across multiple components', async () => {
      ...
      logger.info('Request started', { operation: 'test' }, 'api-gateway', requestId);
      logger.debug('Path validation started', { path: '/test/path' }, 'path-handler', requestId);
      logger.info('Permission check completed', { canRead: true }, 'permission-service', requestId);
      logger.info('Request completed', { duration: '100ms' }, 'api-gateway', requestId);
      ...
      const timestamps = logEntries.map((entry) => new Date(entry.timestamp).getTime());
    ...
    it('should handle concurrent requests with separate correlation IDs', async () => {
      ...
      await Promise.all(
        operations.map(async ({ requestId, operation }) => {
          logger.setRequestId(requestId);
          logger.info('Operation started', { operation }, 'test-component', requestId);
  ...
  describe('External Logging Service Integration', () => {
    ...
    it('should handle external service connection failures gracefully', async () => {
      ...
      expect(() => {
        ...
        logger.error('Test error for external service', new Error('Test error'));
      }).not.toThrow();
    });
  });
  ...
  describe('Load Testing for Logging Performance', () => {
    it('should handle high-volume logging without performance degradation', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        if (i % 200 === 0) {
          logger.error(`Error at iteration ${i}`, new Error(`Test error ${i}`), { iteration: i });
        }
      }
