...
export class SystemResourceMonitor extends EventEmitter {
  ...
  public static getInstance(thresholds?: Partial<ResourceThresholds>): SystemResourceMonitor {
  ...
  public startMonitoring(intervalMs = 5000): void {
    ...
    console.log(`📊 Starting system resource monitoring (interval: ${intervalMs}ms)`);
  ...
  public async isSystemUnderStress(): Promise<{
    ...
    if (metrics.memory.usagePercent > this.thresholds.memory.critical) {
      reasons.push(`Critical memory usage: ${metrics.memory.usagePercent.toFixed(1)}%`);
      severity = 'critical';
    ...
    try {
      ...
      if (process.platform === 'win32') {
        ...
        try {
          ...
          usage = match ? Number.parseInt(match[1], 10) : 0;
        } catch {
          usage = 0;
        }
      } else {
        ...
        try {
          ...
          usage = match ? Number.parseFloat(match[1]) : 0;
        } catch {
          usage = 0;
        }
  ...
  private async getMemoryMetrics(): Promise<SystemMetrics['memory']> {
    try {
      if (process.platform === 'win32') {
        ...
        const total = totalMatch ? Number.parseInt(totalMatch[1], 10) * 1024 : 0; // Convert KB to bytes
      ...
      if (memLine) {
        ...
        const available = Number.parseInt(parts[6], 10) || free;
        const usagePercent = total > 0 ? (used / total) * 100 : 0;
      ...
      if (process.platform === 'win32') {
        ...
        for (const line of lines) {
          if (line.trim()) {
            ...
            if (processName?.includes('vitest') || processName?.includes('node')) {
  ...
  private async getDiskMetrics(): Promise<SystemMetrics['disk']> {
    try {
      if (process.platform === 'win32') {
        ...
        const available = match ? Number.parseInt(match[1], 10) / 1024 / 1024 / 1024 : 0; // GB
      ...
      if (dataLine) {
        ...
        const usage = Number.parseInt(usageStr.replace('%', ''), 10) || 0;
        const available = Number.parseFloat(availableStr.replace(/[^\d.]/g, '')) || 0;
...
export const stopResourceMonitoring = () => systemResourceMonitor.stopMonitoring();

export const getCurrentMetrics = () => systemResourceMonitor.getCurrentMetrics();

export const getConcurrencyRecommendation = () =>
  systemResourceMonitor.getConcurrencyRecommendation();

export const isSystemUnderStress = () => systemResourceMonitor.isSystemUnderStress();
