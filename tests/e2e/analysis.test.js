...
import { startTestServer, createTestRepository, waitForAnalysis, TEST_REPOSITORIES, PerformanceMonitor, } from './setup';
describe('Repository Analysis E2E Tests', () => {
    ...
    describe('Single Repository Analysis', () => {
        test('should analyze JavaScript project successfully', async () => {
            ...
            expect(analysis.result.dependencies.production).toEqual(expect.arrayContaining([
        ...
        test('should analyze React TypeScript project', async () => {
            ...
            expect(analysis.result.structure.keyFiles).toEqual(expect.arrayContaining([
    ...
    describe('Batch Analysis', () => {
        test('should process multiple repositories', async () => {
            ...
            const reactRepo = await createTestRepository('batch-react', TEST_REPOSITORIES.reactTypeScript);
            const pythonRepo = await createTestRepository('batch-python', TEST_REPOSITORIES.pythonProject);
            ...
            while (retries > 0) {
                try {
                    const batchResponse = await axios.get(`${server.baseUrl}/api/batch/${response.data.batchId}`);
        ...
        test('should handle partial failures in batch', async () => {
            const jsRepo = await createTestRepository('batch-partial-js', TEST_REPOSITORIES.simpleJavaScript);
    ...
    describe('Repository Index and Search', () => {
        ...
        test('should search repositories by language', async () => {
            ...
            if (searchResponse.data.length > 0) {
                expect(searchResponse.data.every((repo) => repo.languages.includes('JavaScript'))).toBe(true);
            }
        });
        test('should search repositories by framework', async () => {
            ...
            expect(searchResponse.status).toBe(200);
            expect(searchResponse.data).toEqual(expect.arrayContaining([
                expect.objectContaining({
    ...
    describe('Performance Tests', () => {
        ...
        test('should handle concurrent analysis requests', async () => {
            ...
            const analysisPromises = repos.map((repo) => axios.post(`${server.baseUrl}/api/analyze`, {
                path: repo.path,
                options: { mode: 'quick' },
            }));
            const responses = await Promise.all(analysisPromises);
            // Wait for all analyses to complete
            const analyses = await Promise.all(responses.map((response) => waitForAnalysis(server.baseUrl, response.data.analysisId)));
