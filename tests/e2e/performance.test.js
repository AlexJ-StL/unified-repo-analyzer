...
import { startTestServer, createTestRepository, waitForAnalysis, PerformanceMonitor, } from './setup';
...
`,
            ...
            const analyses = await Promise.all(responses.map((response) => waitForAnalysis(server.baseUrl, response.data.analysisId, 30000)));
...
const data${i} = ${JSON.stringify(Array.from({ length: 1000 }, (_, j) => ({
                    id: j,
                    name: `item-${j}`,
                    value: Math.random(),
                    nested: {
