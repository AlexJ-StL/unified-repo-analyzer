...
import { createTestRepository, TEST_REPOSITORIES, PerformanceMonitor, } from './setup';
describe('CLI End-to-End Tests', () => {
    ...
    describe('Batch Command', () => {
        test('should process multiple repositories', async () => {
            ...
            const jsRepo = await createTestRepository(join(batchDir, 'js-project'), TEST_REPOSITORIES.simpleJavaScript);
            const reactRepo = await createTestRepository(join(batchDir, 'react-project'), TEST_REPOSITORIES.reactTypeScript);
            const pythonRepo = await createTestRepository(join(batchDir, 'python-project'), TEST_REPOSITORIES.pythonProject);
        ...
        test('should handle partial failures in batch', async () => {
            ...
            await mkdir(batchDir, { recursive: true });
            const validRepo = await createTestRepository(join(batchDir, 'valid-project'), TEST_REPOSITORIES.simpleJavaScript);
    ...
    describe('Search Command', () => {
        test('should search repositories by language', async () => {
            ...
            const reactRepo = await createTestRepository('search-react', TEST_REPOSITORIES.reactTypeScript);
            ...
            if (output.length > 0) {
                expect(output.every((repo) => repo.languages.some((lang) => ['JavaScript', 'TypeScript'].includes(lang)))).toBe(true);
            }
        });
        test('should search repositories by framework', async () => {
            const reactRepo = await createTestRepository('search-framework', TEST_REPOSITORIES.reactTypeScript);
