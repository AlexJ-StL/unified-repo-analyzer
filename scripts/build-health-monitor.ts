...
class BuildHealthMonitor {
    ...
    try {
      ...
      if (!existsSync(nodeModules)) {
        ...
        if (!existsSync(binDir)) {
          ...
          suggestions.push("Run 'bun install --force' to restore binary mappings");
        }
        ...
        try {
          ...
          const packageCount = packages.filter((p: string) => !p.startsWith('.')).length;
          ...
          if (packageCount === 0) {
            ...
            suggestions.push('Consider dependency audit for large package count');
          }
        } catch {
          score -= 20;
  ...
  private async checkTypeScriptHealth(): Promise<void> {
    ...
    try {
      ...
      const tsVersionResult = await this.runCommand('bunx', ['tsc', '--version'], {
        timeout: 10000,
      });
      ...
      if (compileResult.success) {
        ...
        const errorCount = (compileResult.stderr?.match(/error TS\d+:/g) || []).length;
        if (errorCount > 0) {
    ...
    try {
      ...
      if (existsSync(packageJsonPath)) {
        ...
        if (missingScripts.length > 0) {
          ...
          suggestions.push(`Add missing build scripts: ${missingScripts.join(', ')}`);
        }
      ...
      // Test build process
      const buildTestResult = await this.runCommand('bun', ['run', 'build:shared'], {
        timeout: 120000,
      });
  ...
  private async checkEnvironmentHealth(): Promise<void> {
    ...
    try {
      ...
      if (majorVersion < 18) {
        ...
        score -= 10;
        status = 'warning';
        suggestions.push('Consider upgrading to Node.js 20+ for better performance');
      }
  ...
  private async checkPerformanceHealth(): Promise<void> {
    ...
    try {
      ...
      const buildResult = await this.runCommand('bun', ['run', 'build:shared'], {
        timeout: 120000,
      });
      ...
      if (buildResult.success) {
        ...
        score -= 50;
        status = 'critical';
        suggestions.push('Build performance cannot be measured due to build failure');
      }
      ...
      for (const file of largeFiles) {
        if (existsSync(file)) {
          try {
            ...
            if (sizeMB > 10) {
              score -= 5;
              suggestions.push(`Large lock file (${Math.round(sizeMB)}MB) may impact performance`);
            }
          } catch {
            // Ignore stat errors
          }
  ...
  private async checkMaintenanceHealth(): Promise<void> {
    ...
    try {
      ...
      if (existsSync(packageJsonPath)) {
        ...
        const totalDeps = Object.keys(devDeps).length + Object.keys(deps).length;
      ...
      if (oldArtifacts > 0) {
        score -= 5;
        suggestions.push('Consider rebuilding packages with old build artifacts');
      ...
      for (const logFile of logFiles) {
        if (existsSync(logFile)) {
          ...
          try {
            ...
            if (sizeMB > 10) {
              score -= 5;
              suggestions.push(`Large log file (${Math.round(sizeMB)}MB): ${logFile}`);
            }
          } catch {
            // Ignore stat errors
          }
  ...
  private generateHealthReport(): HealthReport {
    ...
    if (critical > 0) {
      recommendations.push('Address critical issues immediately to restore build functionality');
    }
    if (warnings > 0) {
      recommendations.push('Review warning items to prevent future issues');
    }
    if (overallScore < 80) {
      recommendations.push("Run 'bun run recovery:full' to address multiple issues");
    }
    if (overallScore >= 90) {
      recommendations.push('Build system is in excellent health - maintain current practices');
    }
  ...
  private displayHealthReport(report: HealthReport): void {
    ...
    // Overall status
    const statusIcon =
      report.overallStatus === 'healthy' ? '🟢' : report.overallStatus === 'warning' ? '🟡' : '🔴';
