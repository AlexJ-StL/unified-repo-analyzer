...
class DeploymentVerifier {
  ...
  async run(): Promise<void> {
    ...
    if (criticalFailures > 0) {
      ...
      console.log(`${criticalFailures} critical test(s) failed. Deployment should be rolled back.`);
      process.exit(1);
    } else if (failedTests > 0) {
      console.log('\n⚠️  DEPLOYMENT VERIFICATION COMPLETED WITH WARNINGS');
      console.log(`${failedTests} non-critical test(s) failed. Monitor closely.`);
    } else {
  ...
  private async testPathValidationApi(): Promise<TestResult> {
    ...
    for (const testPath of testPaths) {
      try {
        ...
        if (response.statusCode === 200) {
          const data = JSON.parse(response.body);
          if (Object.hasOwn(data, 'isValid') && Object.hasOwn(data, 'normalizedPath')) {
  ...
  private async testLoggingApi(): Promise<TestResult> {
    ...
    try {
      // Test log retrieval
      const logsResponse = await this.makeHttpRequest(`${this.baseUrl}/api/logs?limit=10`);
      ...
      if (logsResponse.statusCode === 200) {
        ...
        results.push(`✗ Log retrieval endpoint: HTTP ${logsResponse.statusCode}`);
        allPassed = false;
      }

      // Test log configuration
      const configResponse = await this.makeHttpRequest(`${this.baseUrl}/api/logs/config`);
      ...
      if (configResponse.statusCode === 200) {
        ...
        results.push(`✗ Log configuration endpoint: HTTP ${configResponse.statusCode}`);
        allPassed = false;
      }
    } catch (error) {
    ...
    return {
      passed: allPassed,
      message: allPassed ? 'Logging API endpoints working correctly' : 'Logging API has issues',
      details: results,
    };
  }
  ...
  private async testConfigurationLoading(): Promise<TestResult> {
    ...
    try {
      ...
      const response = await this.makeHttpRequest(`${this.baseUrl}/api/logs/config`);
  ...
  private async testLogFileCreation(): Promise<TestResult> {
    ...
    try {
      ...
      if (fs.existsSync(logDir)) {
        ...
        if (logFiles.length > 0) {
          ...
          results.push('⚠ No log files found (may be normal for new deployment)');
        }
      } else {
    ...
    return {
      passed: allPassed,
      message: allPassed ? 'Log file creation working correctly' : 'Log file creation has issues',
      details: results,
    };
  }

  private async testBackwardCompatibility(): Promise<TestResult> {
    const legacyEndpoints = ['/api/analyze', '/api/repositories', '/api/export'];
    ...
    for (const endpoint of legacyEndpoints) {
      try {
        ...
        if (response.statusCode === 404) {
          ...
          results.push(`✓ ${endpoint}: Endpoint exists (HTTP ${response.statusCode})`);
        }
      } catch (error) {
  ...
  private async testPerformanceBaseline(): Promise<TestResult> {
    ...
    try {
      ...
      for (const endpoint of endpoints) {
        const startTime = Date.now();
        const _response = await this.makeHttpRequest(`${this.baseUrl}${endpoint}`);
        const duration = Date.now() - startTime;
    ...
    return {
      passed: allPassed,
      message: allPassed ? 'Performance baseline acceptable' : 'Performance issues detected',
      details: results,
    };
  }
