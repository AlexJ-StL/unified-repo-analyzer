...
class CoverageSummary {
  ...
  private checkCoverageInfrastructure(): boolean {
    ...
    try {
      ...
      const hasScripts =
        packageJson.scripts?.['test:coverage'] && packageJson.scripts['test:coverage:analysis'];
  ...
  private async identifyIssuesAndRecommendations(status: CoverageStatus): Promise<void> {
    ...
    if (!existsSync(this.reportsDir)) {
      ...
      status.recommendations.push('Create reports directory: mkdir coverage-reports');
    }
    ...
    if (existsSync(packageJsonPath)) {
      try {
        ...
        if (!hasCoverageProvider) {
          status.issues.push('Coverage provider (@vitest/coverage-v8) is not installed');
          status.recommendations.push('Install coverage provider: bun add -d @vitest/coverage-v8');
        }
      } catch {
  ...
  private async generateReport(status: CoverageStatus): Promise<void> {
    ...
    const report = {
      ...
      status: {
        ...
        overall: status.configured && status.working ? 'good' : 'needs-attention',
      },
  ...
  private getNextSteps(status: CoverageStatus): string[] {
    ...
    if (!status.configured) {
      ...
      steps.push('3. Re-run validation: bun scripts/coverage-validation.ts full');
    }
