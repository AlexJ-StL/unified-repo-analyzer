...
class BuildDoctor {
	...
	private async checkProjectStructure(): Promise<void> {
			...
			try {
				...
				if (packageData.workspaces) {
					...
					this.addDiagnostic({
						...
						status: "warning",
						message:
							"Root package.json exists but no workspace configuration found",
		...
		if (existsSync(tsConfig)) {
			...
			this.addDiagnostic({
				...
				suggestions: [
					"Create root tsconfig.json for project-wide TypeScript settings",
				],
			});
		}
	}
	...
	private async checkTypeScriptConfiguration(): Promise<void> {
		...
		try {
			...
			if (result.success) {
				...
				const tsErrors = result.stderr
					? ErrorAnalyzer.parseTypeScriptErrors(result.stderr)
					: [];
			...
			this.addDiagnostic({
				...
				suggestions: [
					"Check if TypeScript is installed and build scripts are configured",
				],
			});
		}
	}
	...
	private async checkEnvironment(): Promise<void> {
		...
		if (availableMemoryMB >= 512) {
			...
			this.addDiagnostic({
				...
				suggestions: [
					"Consider increasing Node.js memory limit with --max-old-space-size",
				],
			});
		}
	...
	private generateDiagnosticReport(): void {
		...
		for (const status of statusOrder) {
			...
			for (const item of items) {
				console.log(
					`${statusIcons[item.status]} ${item.name}: ${item.message}`,
				);
	...
	async executeRecoveryActions(): Promise<void> {
		const automatedActions = this.recoveryActions.filter(
			(a) => a.automated && a.risk === "low",
		);
