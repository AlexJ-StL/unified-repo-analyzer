...
class BuildMonitor {
	...
	async startMonitoring(): Promise<void> {
		...
		console.log(
			`   Auto-fix: ${this.config.autoFix.enabled ? "enabled" : "disabled"}`,
		);
	...
	private setupFileWatchers(): void {
		...
		for (const pattern of this.config.watchFiles) {
			if (pattern.includes("*")) {
				...
				const eventType = pattern.includes("package.json")
					? "dependency_change"
					: "config_change";
	...
	private async handleFileChange(
		...
		const alert: MonitorAlert = {
			...
			suggestions: [
				"Run health check to verify changes",
				"Consider rebuilding affected packages",
			],
	...
	private async performHealthCheck(): Promise<void> {
		...
		try {
			...
			const result = await this.runCommand(
				"bun",
				["run", "scripts/build-health-monitor.ts"],
				{ timeout: 120000 },
			);
			...
			this.healthHistory = this.healthHistory.filter(
				(h) => h.timestamp.getTime() > oneDayAgo,
			);
			...
			if (healthScore < this.config.healthThreshold) {
				const alert: MonitorAlert = {
					...
					details:
						result.stderr || "Check build-health-report.json for details",
					suggestions: [
			...
			console.log(
				`   Health score: ${healthScore}% (previous: ${previousScore}%)`,
			);
		} catch (error) {
			const alert: MonitorAlert = {
	...
	private async sendWebhookAlert(alert: MonitorAlert): Promise<void> {
		...
		console.log(
			`📡 Webhook alert: ${alert.title} (webhook integration not implemented)`,
		);
	}
