...
class ConfigMigrator {
	...
	async run(): Promise<void> {
		...
		try {
			...
			if (configFiles.length === 0) {
				console.log(
					"ℹ️  No configuration files found. Creating default configuration...",
				);
			...
			console.log(
				"   4. Remove backup files when satisfied with the migration",
			);
		} catch (error) {
	...
	private async migrateConfigFile(configPath: string): Promise<void> {
		...
		console.log(
			`   📋 Backup created: ${path.relative(this.projectRoot, backupPath)}`,
		);
	...
	private migratePathHandling(config: Record<string, string>): MigrationResult {
		...
		if (config["MAX_PATH_LENGTH"] && !config["WINDOWS_LONG_PATH_SUPPORT"]) {
			const maxLength = parseInt(config["MAX_PATH_LENGTH"]);
			if (maxLength > 260) {
				config["WINDOWS_LONG_PATH_SUPPORT"] = "true";
				result.changes.push(
					"Enabled WINDOWS_LONG_PATH_SUPPORT based on MAX_PATH_LENGTH",
				);
			}
		}
	...
	private migratePerformanceConfig(
		config: Record<string, string>,
	): MigrationResult {
		const result: MigrationResult = {
			success: true,
