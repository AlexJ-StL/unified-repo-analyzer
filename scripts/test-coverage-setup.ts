...
async function testCoverageSetup(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "   ⚠️  Coverage collection had issues, but setup is complete"
      );
    }
    ...
    console.log(
      "• bun run test:coverage:analysis - Generate detailed analysis"
    );
    ...
    console.log(
      "• bun run test:coverage:full - Run complete coverage workflow"
    );
  } catch (error) {
    console.error("❌ Coverage setup test failed:", error);
    process.exit(1);
  }
}
